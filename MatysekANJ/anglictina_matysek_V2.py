...
# Fronta pro anglický TTS — zabraňuje překrývání hlasů
...
class Button:
    ...
    def draw(self, screen: pygame.Surface, font: pygame.font.Font, mouse_pos: tuple[int, int]):
        ...
        # Kontrastní rámeček: pro tmavé pozadí světlý, pro světlé tmavý
        brightness = sum(self.bg) / 3
        border_col = (200, 215, 230) if brightness < 100 else (35, 55, 80)
        pygame.draw.rect(screen, border_col, self.rect, width=2, border_radius=14)
        txt = font.render(self.text, True, self.fg)
        screen.blit(txt, txt.get_rect(center=self.rect.center))


FRUIT_VEG_WORDS = {
    "apple":      {"cz": "jablko",    "emoji": "🍎", "color": (220, 50,  50)},
    "banana":     {"cz": "banan",     "emoji": "🍌", "color": (255, 220, 50)},
    "orange":     {"cz": "pomeranc",  "emoji": "🍊", "color": (255, 150, 30)},
    "strawberry": {"cz": "jahoda",    "emoji": "🍓", "color": (220, 40,  80)},
    "grape":      {"cz": "hrozen",    "emoji": "🍇", "color": (140, 60, 180)},
    "watermelon": {"cz": "meloun",    "emoji": "🍉", "color": (60,  170, 80)},
    "lemon":      {"cz": "citron",    "emoji": "🍋", "color": (255, 240, 60)},
    "cherry":     {"cz": "tresne",    "emoji": "🍒", "color": (200, 30,  60)},
    "pear":       {"cz": "hruska",    "emoji": "🍐", "color": (160, 200, 80)},
    "pineapple":  {"cz": "ananas",    "emoji": "🍍", "color": (255, 200, 40)},
    "carrot":     {"cz": "mrkev",     "emoji": "🥕", "color": (255, 130, 30)},
    "tomato":     {"cz": "rajce",     "emoji": "🍅", "color": (220, 50,  50)},
    "corn":       {"cz": "kukurice",  "emoji": "🌽", "color": (255, 220, 50)},
    "broccoli":   {"cz": "brokolice", "emoji": "🥦", "color": (50,  160, 70)},
    "cucumber":   {"cz": "okurka",    "emoji": "🥒", "color": (80,  180, 80)},
    "mushroom":   {"cz": "houby",     "emoji": "🍄", "color": (160, 100, 60)},
}
...
class AnglictinaMatysekApp:
    def __init__(self):
        ...
        self.fruit_round: dict = {}
        self.alphabet_round: dict = {}
    ...
    def _home_buttons(self, w: int, h: int) -> list[tuple[Button, str]]:
        ...
        items = [
            ...
            ("Ovoce", "fruit",    (255, 236, 210)),
            ("Abeceda", "alphabet",(230, 220, 255)),
            ("Odmena", "reward",  (214, 237, 255)),
        ]
        out = []
    ...
    def _switch_scene(self, new_scene: str):
        ...
        elif new_scene == "fruit":
            self.fruit_new_round()
            owl_speak_czech("Teď budeme hledat ovoce a zeleninu.")
        elif new_scene == "alphabet":
            self.alphabet_new_round()
            owl_speak_czech("Teď budeme trénovat písmenka.")
    ...
    def _queue_speak(self, delay: float, text: str, voice: str = "Samantha"):
        """Naplánuje anglický TTS po zadaném zpoždění (neblokuje)."""
        if delay > 0:
            def _do():
                speak_english(text, voice)
            # Použijeme thread sleep aby neblokoval pygame loop
            threading.Thread(target=lambda: (time.sleep(delay), speak_english(text, voice)), daemon=True).start()
        else:
            speak_english(text, voice)
    ...
    def colors_new_round(self):
        ...
        self.feedback = ""
        # Zpoždění: počkáme až sova domluví úvod, pak teprve řekneme barvu
        self._queue_speak(1.4, target)
    ...
    def numbers_new_round(self):
        ...
        self.feedback = ""
        # Fronta zajistí: "How many" → pauza → číslo, bez překryvu
        self._queue_speak(1.4, "How many")
        self._queue_speak(0, NUMBER_WORDS[count])
    ...
    def mix_new_round(self):
        ...
        self.feedback = ""
        t = cards[target_idx]
        self._queue_speak(1.4, "Find " + NUMBER_WORDS[t["count"]] + " " + t["color"])
    ...
    def animals_new_round(self):
        ...
        self._queue_speak(1.4, "Click: " + target["word"])
    ...
    def fruit_new_round(self):
        keys = list(FRUIT_VEG_WORDS.keys())
        options = random.sample(keys, 4)
        target_idx = random.randrange(4)
        self.fruit_round = {"options": options, "target_idx": target_idx}
        self.feedback = ""
        self._queue_speak(1.4, options[target_idx])

    def alphabet_new_round(self):
        import string
        # Pro 5leté dítě: velká písmena A-Z, 4 možnosti
        all_letters = list(string.ascii_uppercase)
        options = random.sample(all_letters, 4)
        target = random.choice(options)
        self.alphabet_round = {"options": options, "target": target}
        self.feedback = ""
        self._queue_speak(1.4, target)

    def draw_fruit(self, w: int, h: int, mouse_pos: tuple[int, int]):
        target_key = self.fruit_round["options"][self.fruit_round["target_idx"]]
        target_info = FRUIT_VEG_WORDS[target_key]
        self.screen.blit(self.font_h1.render("Hra: Ovoce a zelenina", True, TEXT), (40, 92))
        self.screen.blit(self.font_h2.render(f"Klikni na: {target_key.upper()}", True, TEXT), (40, 140))
        self._draw_feedback(185)
        ...
        card_w = int((w - margin * 2 - gap) / 2)
        card_h = int((h - 300 - gap) / 2)
        for i, key in enumerate(self.fruit_round["options"]):
            info = FRUIT_VEG_WORDS[key]
            row = i // 2
            col = i % 2
            rect = pygame.Rect(margin + col * (card_w + gap), 240 + row * (card_h + gap), card_w, card_h)
            # Světlé pozadí odvozené od barvy ovoce
            bg = tuple(min(255, c + 160) for c in info["color"])
            pygame.draw.rect(self.screen, bg, rect, border_radius=14)
            border_col = tuple(max(0, c - 40) for c in info["color"])
            pygame.draw.rect(self.screen, border_col, rect, 2, border_radius=14)

            # Velký emoji symbol uprostřed
            emoji_surf = self.font_big.render(info["emoji"], True, info["color"])
            # Fallback: emoji fonty nemusí fungovat na všech systémech — zobrazíme barevný kruh
            circle_r = int(min(card_w, card_h) * 0.28)
            cx = rect.centerx
            cy = rect.y + int(card_h * 0.42)
            pygame.draw.circle(self.screen, info["color"], (cx, cy), circle_r)
            pygame.draw.circle(self.screen, (40, 55, 80), (cx, cy), circle_r, 3)
            # Iniciála uprostřed kruhu
            init = self.font_big.render(key[0].upper(), True, (255, 255, 255))
            self.screen.blit(init, init.get_rect(center=(cx, cy)))

            # Název pod kruhem
            label = self.font_m.render(key.upper(), True, (30, 40, 60))
            self.screen.blit(label, label.get_rect(center=(rect.centerx, rect.y + card_h - 30)))
            # CZ název
            cz = self.font_s.render(f"({info['cz']})", True, (80, 90, 110))
            self.screen.blit(cz, cz.get_rect(center=(rect.centerx, rect.y + card_h - 10)))

            if rect.collidepoint(mouse_pos):
                pygame.draw.rect(self.screen, (70, 130, 200), rect, 3, border_radius=14)
            rects.append((rect, i))
        return rects

    def draw_alphabet(self, w: int, h: int, mouse_pos: tuple[int, int]):
        target = self.alphabet_round["target"]
        options = self.alphabet_round["options"]
        self.screen.blit(self.font_h1.render("Hra: Abeceda", True, TEXT), (40, 92))
        self.screen.blit(self.font_h2.render(f"Najdi pismeno: {target}", True, BLUE), (40, 140))
        self._draw_feedback(185)

        # Velké ukázkové písmeno vlevo
        demo_rect = pygame.Rect(40, 240, int(w * 0.35), int(h * 0.55))
        pygame.draw.rect(self.screen, (230, 240, 255), demo_rect, border_radius=18)
        pygame.draw.rect(self.screen, (100, 140, 220), demo_rect, 3, border_radius=18)
        big_letter = pygame.font.SysFont("arial", 180, bold=True).render(target, True, BLUE)
        self.screen.blit(big_letter, big_letter.get_rect(center=demo_rect.center))
        ...
        bh = int(h * 0.22)
        gap = 16
        colors_cycle = [(255, 224, 239), (255, 247, 191), (220, 248, 255), (224, 248, 222)]
        for i, letter in enumerate(options):
            row = i // 2
            col = i % 2
            rect = pygame.Rect(bx + col * (bw + gap), 260 + row * (bh + gap), bw, bh)
            bg = colors_cycle[i % 4]
            pygame.draw.rect(self.screen, bg, rect, border_radius=14)
            pygame.draw.rect(self.screen, (80, 100, 140), rect, 2, border_radius=14)
            lbl = pygame.font.SysFont("arial", 90, bold=True).render(letter, True, (30, 50, 100))
            self.screen.blit(lbl, lbl.get_rect(center=rect.center))
            if rect.collidepoint(mouse_pos):
                pygame.draw.rect(self.screen, (70, 130, 200), rect, 4, border_radius=14)
            rects.append((rect, i))
        return rects

    def draw_reward(self, w: int, h: int):
        self.screen.blit(self.font_h1.render("Odmena pro Matyska", True, TEXT), (40, 92))
    ...
    def run(self):
        ...
        while running:
            ...
            if self.scene == "home":
                ...
                elif self.scene == "fruit":
                    for rect, idx in self.draw_fruit(w, h, mouse_pos):
                        clickables.append((rect, "fruit_pick", idx))
                elif self.scene == "alphabet":
                    for rect, idx in self.draw_alphabet(w, h, mouse_pos):
                        clickables.append((rect, "alpha_pick", idx))
                ...
                if event.type == pygame.QUIT:
                    ...
                    for rect, action, idx in clickables:
                        if not rect.collidepoint((mx, my)):
                            continue
                        if action in ("home", "colors", "numbers", "animals", "mix", "fruit", "alphabet", "reward"):
                        ...
                        elif action == "fruit_pick" and idx is not None:
                            target_idx = self.fruit_round["target_idx"]
                            options = self.fruit_round["options"]
                            if idx == target_idx:
                                speak_english(options[target_idx])
                                self._answer(True, 12, 2, random.choice(PRAISES), "", self.fruit_new_round)
                            else:
                                picked_cz = FRUIT_VEG_WORDS[options[idx]]["cz"]
                                correct = options[target_idx]
                                self._answer(False, 12, 2, "", f"To je {picked_cz}. Hledame {correct.upper()}.", self.fruit_new_round)
                        elif action == "alpha_pick" and idx is not None:
                            target = self.alphabet_round["target"]
                            options = self.alphabet_round["options"]
                            if options[idx] == target:
                                speak_english(target)
                                self._answer(True, 10, 1, random.choice(PRAISES), "", self.alphabet_new_round)
                            else:
                                self._answer(False, 10, 1, "", f"To je {options[idx]}. Hledame {target}.", self.alphabet_new_round)
                        break
