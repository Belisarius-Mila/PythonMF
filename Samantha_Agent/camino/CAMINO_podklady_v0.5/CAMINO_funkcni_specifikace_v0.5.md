# Camino — detailní funkční specifikace v0.5
...
Tato v0.5 je aktuální pracovní specifikace. Zachovává potvrzená rozhodnutí U01–U11 z v0.3 a konkretizuje obrazovky, akce, výchozí hodnoty a chování při chybě. Starší soubory zůstávají beze změny. V případě rozporu má pro další vývoj přednost v0.5; historické varianty se nemají vracet do kódu.

**U** označuje rozhodnutí potvrzené Mílou. **D** je nové návrhové rozhodnutí Samanthy, které lze změnit záznamem důvodu, nikoli vydávat za dodatečně schválené uživatelem. **F** je funkční pravidlo; **UI** označuje obrazovku. Testy **T001–T095** jsou v samostatném souboru `CAMINO_akceptacni_testy_v0.5.md`. Etapy **C00–C11** jsou v `CAMINO_Codex_v0.5.md`.
...
| U12 | Během pouti má Jana z vlastního MacBooku nebo iPhonu vidět inteligentní read-only přehled fotografií, videí a komentářů z domácího Mac serveru. | Camino Viewer se stává P0 před cestou; musí fungovat přes soukromý přístup bez veřejného webu. |
| U13 | Viewer se má aktualizovat automaticky bez Mílovy večerní redakce; běžné „Do deníku“ je pro Viewer způsobilé, „Jen pro mě“ je absolutně vyloučeno. | Vzniká samostatná viewer-safe projekce a automatická publikace po změnách i kontrolní regenerace. |
| U14 | Domácí otevřený MacBook má po dobu cesty sloužit jako server a Jana může používat stávající Cockpit. | Před cestou je nutný audit napájení/uspávání, restartu, Tailscale, spravovaného startu a bezpečné integrace Cockpitu. |

### 1.2 Vývojový rozsah

**P0 — jádro před poutí:** offline fotografování, video, audio, textové doplnění, import, Moment a den, zámek, lokální přehled a přehrávání, nouzový export, soukromý upload, obnova přenosů, druhá záloha a zkouška obnovy, přepisy a automatický osobní deník, jednoduchá oprava textu, provozní stav, připravená instalace a **Camino Viewer pro Janu**: automatický read-only web s bezpečnými náhledy fotografií, video proxy, texty a původním povoleným audiem přes Tailscale.

**P1 — nepovinné rozšíření:** jednorázový vybraný text a fotografie dalším členům rodiny přes systémové sdílení. Jana již pro běžné sledování cesty používá P0 Camino Viewer; P1 nesmí blokovat dokončení P0.
...
**Mimo rozsah:** více autorů, veřejné odkazy na server, Tailscale Funnel, živé sledování polohy rodinou, automatické rozesílání mimo Viewer, denní povinné filmy, generované záběry a hlas, automatická obrazová analýza všech médií, integrace sociálních sítí, vlastní VPN, navigace na trase.

### 1.3 Nová pracovní rozhodnutí této revize

| ID | Rozhodnutí Samanthy | Důvod |
|---|---|---|
| D01 | V P0 se obsah a soukromí upravují pouze v hlavním iPhonu; vlastnický web i Camino Viewer jsou read-only. | Méně konfliktů a žádná druhá povinná pracovní plocha. |
...
| D11 | Camino Viewer je samostatná webová služba; Cockpit jen zobrazuje stav a odkaz na Viewer. | Selhání Cockpitu nesmí znepřístupnit deník, pokud Viewer a Tailscale běží. |
| D12 | Viewer používá odvozené náhledy: fotografie thumbnail/preview, video poster + 720p H.264/AAC proxy; originály se běžně neservírují. | Rychlé zobrazení na iPhonu/Macu a ochrana originálů. |
| D13 | Viewer se přegeneruje po relevantní změně a navíc kontrolně večer a ráno; pracovní časy jsou 23:30 a 07:00 místního času serveru. | Bez Mílovy ruční večerní práce a s opravou pozdě doručených dat. |
| D14 | Viewer je dostupný jen přes autorizovaný tailnet/Tailscale Serve; žádný Funnel ani veřejný port. | Soukromé rodinné prohlížení bez publikace na internetu. |
| D15 | Viewer kopie fotografií nemají přesnou GPS ani nepotřebná EXIF/XMP metadata; stránka přesnou polohu implicitně neukazuje. | Kopie uložená z Vieweru nemá zbytečně nést citlivou polohu. |
| D16 | Aktivní Viewer build se publikuje atomicky z explicitní `viewer_safe` projekce; server při mediálním requestu znovu kontroluje aktuální povolenost známou serveru. | Stará HTML/cache nesmí obejít pozdější zámek, který už server přijal. |

Žádné další produktové odpovědi nejsou pro tuto specifikaci nutné; U12–U14 nově uzavírají požadavek na Viewer a domácí server. Hardware serveru, hodinky a verze systémů nejsou domyšlené; řeší je audit. Rozpočet AI se řeší až před placeným pilotem.
...
| UI13 | Správa/owner web na Macu | Autorizovaný přístup vlastníka | P0 |
| UI14 | Film a trasa | Samostatné pozdější rozhraní | P2 |
| UI15 | Camino Viewer pro Janu | Tailscale / odkaz z Cockpitu | P0 |
...
## 17. UI13 — správa a owner web na Macu

**F37.** Owner web má v P0 dvě hlavní části: **Osobní deník** a **Stav systému**. Osobní deník zobrazuje i obsah „Jen pro mě“ a je proto pouze pro vlastníka; Jana používá oddělený UI15 Camino Viewer. Stav ukazuje poslední kontakt telefonu, počty známých nedokončených zdrojů, místo, frontu zpracování, poslední zálohu a výsledek poslední zkoušky obnovy.
...
| MediaDerivative | ID, zdrojový Asset + revize receptu, účel/publikum, hash, stav | Rozměry, kodek, poster, metadata sanitizace |
| ViewerBuild | ID/revize buildu, Trip, přesný seznam povolených zdrojových revizí, stav publikace | Generováno, poslední úspěch, čekající položky, verze šablony |
...
```text
ověřený originál ──→ náhledy a mediální metadata ──→ owner galerie
                ├─→ viewer-safe odvozeniny ──→ Camino Viewer
...
| G8 — Camino Viewer | Jana otevře Viewer z jiné sítě na MacBooku i iPhonu; foto/video/audio/text fungují, `Jen pro mě` neunikne, služby se spravovaně vrátí po restartu procesu. |
...
**F73.** C00–C11 se implementují po malých úlohách; v0.5 přidává uvnitř C08 samostatné kroky C08c–C08f pro Viewer. Automatické testy nevolají skutečné placené API a neobsahují osobní média. Každá předávka uvádí skutečně spuštěné testy, výsledek a NEOVĚŘENO pro testy bez zařízení. Během tohoto zpracování specifikace se neprovedl build iOS aplikace, terénní zkouška, skutečný upload na Mílův Mac ani vzdálený Viewer test na Janiných zařízeních.
...
## 25. UI15 — Camino Viewer pro Janu (P0)

**F74. Viewer-safe projekce.** Camino Viewer je read-only rodinný pohled určený Mílovi a Janě. Jeho vstupní množina se sestavuje **před** generováním HTML, souhrnů a mediálních kopií. Způsobilý je pouze Moment, jehož aktuální serverová revize má známé `owner_only=false`, není skrytá a patří do povolené cesty. Výchozí „Do deníku“ je pro Viewer způsobilé; `Jen pro mě` není nikdy vstupem Vieweru.

Owner-only denní souhrn se nesmí použít jako zkratka pro Janin souhrn, ani když právě neobsahuje zjevně tajnou větu. Viewer text vzniká jen z explicitně povolených textových revizí. Nezpůsobilý zdroj se nesmí projevit ani v názvu, počtu vynechaných položek, HTML komentáři, manifestu dostupném Janě, audio playlistu, thumbnailu, posteru nebo cache.

**F75. Struktura a „inteligentní“ náhled.** Úvodní stránka zobrazuje cestu a dny sestupně nebo chronologicky s datem, názvem etapy, titulní fotografií, krátkým viewer-safe souhrnem a počty skutečně dostupných povolených typů. Detail dne je responsivní časová osa Momentů s rozumnými bloky: samostatné fotografie, galerie explicitně propojených fotografií, video s posterem, komentář/úvaha jako text + přehrání původního povoleného audia a zřetelný stav zpracování.

„Inteligentní“ uspořádání v P0 vychází z Momentů, času, explicitních vazeb, hvězdiček a textu. Bez samostatně schválené obrazové analýzy se netvrdí, že AI poznala nejlepší fotografii nebo obsah klipu. Titulní snímek může být hvězdičkový povolený obrázek, jinak první vhodná fotografie dne; výběr je deterministický a dohledatelný.

**F76. Fotografie a videa pro Viewer.** Originál zůstává v archivu. Viewer standardně používá například thumbnail kolem 400 px a preview kolem 1600 px v běžném webovém formátu; přesné hodnoty se upraví podle měření. Viewer kopie zapracuje orientaci do obrazu a odstraní přesnou GPS i nepotřebná EXIF/XMP metadata.

Video má poster a kompatibilní proxy, pracovně 720p H.264/AAC; originál se běžně nenabízí ke stažení ani nepřednačítá. Server podporuje rozsahové čtení potřebné pro posun v přehrávači. Selhání proxy neznehodnotí originál: Viewer ukáže „Video se zpracovává“ nebo „Náhled není dostupný“, ne rozbitý prázdný přehrávač.

**F77. Text a původní hlas.** U komentáře/úvahy Viewer zobrazuje aktuální jemně opravený text, pokud je hotový, a umožní přehrát původní povolené audio. Není-li přepis hotový nebo AI stojí, audio může být dostupné samostatně. Nejasný přepis se nevydává za doslovný. Syntetický hlas se nepoužívá.

Janin denní souhrn je samostatná odvozenina s publikem `viewer`, vzniká jen z viewer-safe textových zdrojů a má přesný seznam vstupních revizí. Výpadek AI nesmí zablokovat fotografie, videa, audio ani starší bezpečný souhrn; pouze se označí stav textu.

**F78. Aktualizace a publikace.** Relevantní změna (nový ověřený zdroj, dokončený přepis/korektura, změna soukromí/skrytí, nový náhled nebo pozdní záznam) zařadí idempotentní rebuild dotčeného dne. Kromě toho proběhne kontrolní reconcile pracovně v 23:30 a 07:00 místního času serveru; časy jsou konfigurovatelné a nejsou důvodem k novému AI volání, pokud se nezměnily textové vstupy.

Nový Viewer build vzniká mimo aktivní publikační strom. Po validaci manifestu, odkazů a povolenosti se aktivní revize přepne atomicky. Neúplný build se nepublikuje jako hotový. Pozdní upload se vloží do skutečného dne podle F40; nezaloží falešný „dnešní“ den podle času serveru.

**F79. Stav a částečnost.** Viewer uvádí „Naposledy aktualizováno“ a po dnech rozlišuje skutečně dostupné položky od položek, které server zná, ale ještě zpracovává. Může zobrazit například „27 fotografií hotovo, 1 video se zpracovává“. Nezná-li server aktuální inventář telefonu, nesmí tvrdit, že už obdržel úplně vše z daného dne.

Počet zamčených/owner-only položek se Janě nesděluje. Stav Vieweru neodhaluje existenci soukromých úvah ani jejich časy.

**F80. Přístup a Cockpit.** Viewer běží jako samostatná lokální webová služba/proces na domácím Macu a je zpřístupněn přes Tailscale Serve pouze autorizovanému tailnetu. Tailscale Funnel, veřejné port-forwarding pravidlo a veřejný anonymní odkaz jsou mimo rozsah. Přesná politika identity/ACL se ověří C00/C06 na skutečném účtu; nedostupná Janina zařízení jsou NEOVĚŘENO, ne automaticky povolená.

Cockpit smí zobrazovat stav služby, poslední synchronizaci/generaci a tlačítko **Otevřít Camino**, ale není jedinou cestou. Jana má také přímou soukromou URL/bookmark. Viewer je read-only: žádná změna textu, soukromí, mazání ani spuštění publikace z Janina zařízení.

**F81. Odvolání a staré cache.** Jakmile server přijme přísnější revizi soukromí/skrytí, okamžitě označí navázané ViewerBuild/MediaDerivative jako nepovolené a jejich mediální endpoint je nevydá ani přes starou URL. Následný rebuild odstraní odkazy a bezpečně uklidí nepovolené deriváty. Pouhé odstranění položky z nového HTML nestačí.

Změna, která zůstává jen v offline telefonu, nemůže být na serveru účinná dříve, než ji server přijme. iPhone proto ukáže, že změna soukromí **čeká na server**, pokud byl daný Moment již dříve pro Viewer způsobilý. Již zobrazenou, uloženou nebo jinak zkopírovanou kopii na cizím zařízení nelze odvolat.

**F82. Provoz Macu.** Produkční běh není závislý na otevřeném Terminálu. Backend, worker a Viewer mají spravovaný start (preferenčně `launchd`) a čitelný health stav; přesná plist konfigurace vznikne až po auditu skutečných cest a uživatele. Mac je během cesty na napájení; displej smí zhasnout, ale audit musí prokázat, že systémový spánek nezastaví požadované služby.

Po restartu se výslovně otestuje, co se stane před a po místním odemknutí s FileVaultem a uživatelskou session. FileVault ani ochrana dat se kvůli pohodlí nevypínají a automatické přihlášení se nezavádí jako skrytá podmínka. Pokud po tvrdém restartu bude nutný místní zásah, provozní plán to řekne před cestou.

**F83. Provozní stav pro Cockpit.** Cockpit/owner stav může zobrazit minimálně: backend/worker/Viewer běží, poslední kontakt iPhonu, poslední úspěšný Viewer build, počet čekajících nezpracovaných veřejně způsobilých položek, volné místo a stav poslední zálohy. Nezobrazuje osobní texty, přesnou GPS, tajné tokeny ani názvy soukromých úvah.

Ruční **Přegenerovat Viewer** je servisní fallback a spouští stejný idempotentní pipeline jako automatika; nesmí obcházet soukromí ani bezdůvodně znovu volat AI.

**F84. Webová implementace a odolnost.** P0 může používat předgenerované HTML/CSS a malé množství JS servírované přes lokální FastAPI. Šablony jsou server-side (např. Jinja2 nebo ekvivalent), text je escapovaný. Fotografie se lazy-loadují; video používá poster a `preload=metadata`; audio standardní ovládání. Viewer nesmí pro obyčejné otevření stránky vyžadovat živé AI volání.

Existující validní build zůstává čitelný při výpadku AI/workeru. Pokud selže generování nového buildu, aktivní starší build se nepřepíše rozbitou verzí a stav jasně ukáže neaktuálnost.

**F85. Předcestovní vzdálená zkouška.** Před 2. říjnem se provede test mimo domácí Wi‑Fi: Míla odešle z iPhonu fotografii, video, komentář/úvahu a jeden `Jen pro mě`; Jana otevře Viewer na svém MacBooku a iPhonu přes vlastní síť/tailnet. Ověří se fotografie, seek videa, text, původní audio, responsivní layout, nepřítomnost soukromého zdroje, pozdní doplnění a stav po restartu serverových procesů.

**F86. Release priorita.** Camino Viewer je od v0.5 součást P0 před poutí, ale nesmí ohrozit bezpečný záznam. Při časovém konfliktu do 2. října se škrtají kosmetika, P1 jednorázové sdílení a P2 film; neškrtá se ochrana `Jen pro mě`, pravdivé stavy, ověřený přenos/záloha ani základní read-only Viewer na Janiných zařízeních.

## 26. Přehled nových parametrů a dosud neověřených bodů
...
| Viewer thumbnail / preview | cca 400 px / 1600 px; webový formát, očištěná GPS/EXIF | C08d + Safari test |
| Viewer video proxy | pracovně 720p H.264/AAC + poster, Range support | C08d + T084 |
| Viewer regenerace | po relevantní změně + reconcile 23:30 a 07:00 místního času serveru | C08c/C08e |
| Viewer přístup | Tailscale Serve, autorizovaný tailnet, žádný Funnel; přímá URL + odkaz z Cockpitu | C00/C06/C08e |
...
**Doprovodné soubory:** testovací scénáře, plán úloh Codexu, krátký `AGENTS.md`, změnový list a čtecí HTML verze. Neobsahují hotový kód aplikace ani nepravdivá potvrzení testů. Autoritativní text specifikace je tento Markdown. V0.5 povyšuje bezpečný read-only Camino Viewer pro Janu do P0; jakákoli starší v0.4 formulace „web pouze pro vlastníka“ se vztahuje jen na owner web UI13, nikoli na Viewer UI15.
