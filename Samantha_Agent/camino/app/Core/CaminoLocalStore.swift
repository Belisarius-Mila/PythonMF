...
/// C04a metadata only. Media bytes remain in RecordingStore's create-only journal.
...
@MainActor public final class CaminoLocalStore {
    ...
    @discardableResult public func beginAudioIntent(
        ...
        if let existing = try object("AudioIntentRecord", id: sessionID, key: "sessionID") {
            ...
            guard intent.kind == kind else { throw LocalStoreError.duplicateIdentity }
            return intent
        }
        guard let trip = try activeTrip() else { throw LocalStoreError.noActiveTrip }
        let privacy = kind == .reflection ? LocalPrivacy.ownerOnly : try newMomentPrivacy()
        ...
        do {
            ...
            row.setValue(UUID(), forKey: "momentID")
    ...
    private static func model() -> NSManagedObjectModel {
        ...
        model.entities = [
            ...
            entity("AudioIntentRecord", [
                ...
                attribute("privacy", text), attribute("finalized", flag),
            ] + stamp(), unique: [["sessionID"], ["momentID"]]),
