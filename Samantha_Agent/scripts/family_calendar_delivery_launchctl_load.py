#!/usr/bin/env python3
"""Preview or explicitly load the dry-run family-calendar LaunchAgent."""
...
from app.family_calendar_delivery_launchctl_load import (  # noqa: E402
    CommandStatusRunner,
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Preview or explicitly load the family-calendar dry-run "
            "LaunchAgent with verification and rollback."
        ),
    )
...
def main(
    ...
    *,
    ...
    try:
        plan = plan_family_calendar_launchctl_load(**plan_kwargs)
    ...
    try:
        result = apply_family_calendar_launchctl_load(plan, **apply_kwargs)
    except FamilyCalendarLaunchctlLoadError as exc:
        print(json.dumps(exc.safe_document(), sort_keys=True), file=stream)
        return 1
    print(
