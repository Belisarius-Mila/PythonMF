...
<dict>
  ...
  <array>
    ...
    <string>$PROJECT_DIR/scripts/cockpit_server.py</string>
    <string>--host</string>
...
existing_pids="$(pgrep -f "[c]ockpit_server.py --host $TAILSCALE_HOST --port $PORT" || true)"
if [[ -n "$existing_pids" ]]; then
  echo "Stopping existing Tailscale Cockpit listener on $TAILSCALE_HOST:$PORT: $existing_pids"
