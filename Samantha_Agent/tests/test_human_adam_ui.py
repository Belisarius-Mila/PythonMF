...
class HumanAdamUiTests(unittest.TestCase):
    ...
    def test_work_help_is_accessible_static_and_covers_semaphore_and_branch_lifecycle(self) -> None:
        ...
        self.assertIn("Fáze 0 až 4: od registrace projektu po nasazení", panel_source)
        self.assertIn("Fáze 0 — registrace projektu a výchozího handoffu", panel_source)
        self.assertIn("úplně nový projekt i o existující oblast", panel_source)
        self.assertIn("Projekt se registruje jen jednou", panel_source)
        self.assertIn("už nejsou nové projekty", panel_source)
    ...
    def test_project_bootstrap_ui_requires_preview_and_exact_confirmation(self) -> None:
        ...
        self.assertIn("POTVRZUJI REGISTRACI PROJEKTU", HUMAN_ADAM_HTML)
        self.assertNotIn("POTVRZUJI ZALOZENI NOVEHO PROJEKTU", HUMAN_ADAM_HTML)
