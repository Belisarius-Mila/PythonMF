...
class CapabilityAuditTests(unittest.TestCase):
    def test_capability_audit_reports_mapped_tools_and_workflow_count(self) -> None:
        ...
        self.assertIn("Capability registry records: 20", text)
        self.assertIn("High-risk capability records: 4", text)
        self.assertIn("Registry-covered agent tools: 17/", text)
        ...
        self.assertIn("- `apply_vyrazeni_leku`", text)
