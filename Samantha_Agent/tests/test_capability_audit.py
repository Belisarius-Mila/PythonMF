...
class CapabilityAuditTests(unittest.TestCase):
    def test_capability_audit_reports_mapped_tools_and_workflow_count(self) -> None:
        ...
        self.assertIn("Capability registry records: 49", text)
        self.assertIn("High-risk capability records: 11", text)
        self.assertIn("Registry-covered agent tools: 46/", text)
        ...
        self.assertIn("Read-only or low-risk missing records: 35", text)
        ...
        self.assertIn("- Read-only or low-risk: 35", text)
