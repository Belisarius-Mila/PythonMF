...
| iPhone Shortcuts / Mobile Input Layer | 2 | active | Důležitá připomenutí mají GitHub write-ahead a přímý Tailscale POST. První iPhone test bezpečně odhalil prázdný URL vstup ještě před zápisem; nevznikla lokální položka ani GitHub Issue. Obě URL vazby jsou opravené a nakonfigurovaná varianta `Samantha – důležité připomenutí 2` je validovaná a podepsaná mimo git. Quick Notes se tímto krokem neměnily. | `technical/iphone_shortcuts_playground.md`; `tvbcp/workstreams/project-cockpit.md` | `handoffs/iphone_shortcuts_freeze_infrastructure_layer_2026_05_25.md`; `handoffs/workstreams/project-cockpit.md` | Importovat opravenou variantu a provést jeden neškodný test s bdícím Macem; potom samostatně test se spícím Macem a převzetím právě jednou. |
