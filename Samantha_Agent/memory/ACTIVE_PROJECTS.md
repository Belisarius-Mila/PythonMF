...
| iPhone Shortcuts / Mobile Input Layer | 2 | active | GitHub write-ahead a přímý Tailscale POST jsou živé; backendová fail-closed ochrana je nasazená na `04e7763` a smoke prošel 5/5. Varianty `3` i `4` při bdícím běhu odeslaly jen `delivery_id` `samantha-`; druhý pokus zůstal bezpečně v otevřené Issue `#5` a lokální index se nezměnil. Varianta `5` proto nepoužívá mezikroky Date/Format Date, ale přímou formátovanou `CurrentDate` magic variable; je validovaná a soukromě podepsaná, ještě však není importovaná ani provozně otestovaná. Quick Notes se neměnily. | `technical/iphone_shortcuts_playground.md`; `tvbcp/workstreams/project-cockpit.md` | `handoffs/iphone_shortcuts_freeze_infrastructure_layer_2026_05_25.md`; `handoffs/workstreams/project-cockpit.md` | Importovat variantu `5`, provést jeden bdící test a ověřit celé nové časové `delivery_id`; teprve potom udělat jeden test se spícím Macem. |
