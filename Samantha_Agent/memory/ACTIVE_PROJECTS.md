...
| ToBeToHave | 2 | active | Kanonická desktopová výuková aplikace je `ToBeTraining/tobe_trenink.py`. Procvičuje to be, to have a to go ze dvou stávajících lokálních CSV, která se při Cockpit integraci nemění. V katalogu Webových aplikací má vlastní allowlistovanou spouštěcí kartu s Pythonem 3.12/Tk 8.6. | `projects/to_be_to_have.md`; `WORKSTREAMS.md` | zatím nematerializováno; stabilní lazy cesta `handoffs/workstreams/project-to-be-to-have.md` | Po nasazení ověřit dlaždici v živém Cockpitu a skutečný start aplikace; další změny obsahu nebo CSV dělat jen na samostatné zadání. |
