...
| Samantha Agent/RAG | 1 | active | P0-P6 zavedly autoritu zdrojů a obsahové narovnání, ale živý audit kalendáře 2026-08-01 odhalil nový drift: potvrzená runtime aktivace se nepropsala do kanonické git-safe paměti a P6 ji bez živého ověření snížilo na `neověřeno`. | `samantha_core.md`; `tvbcp/workstreams/project-samantha-agent-rag.md` | `handoffs/workstreams/project-samantha-agent-rag.md` | P7: navrhnout a otestovat úzkou ochranu proměnlivého provozního stavu — dotaz typu aktivní/běží/připraveno musí použít registrovaný redigovaný live audit, nebo výslovně uvést stáří a nejistotu paměti. |
...
| App-server rozhrani / novy Adam | 1 | active | Human–Adam, kanonické pracovní proudy a globální operace panelu Práce jsou funkční. Dne 2026-08-01 byl odstraněn historický allowlist tří zapisovatelných lazy proudů: všechny zvolitelné aktivní i pozastavené proudy nyní používají stejný jednorázový vývojový režim, zatímco běžný tah bez vývojového záměru zůstává nezapisovací a archivované proudy jsou mimo výběr. Oprava prošla 1263 testy a commit `ef3526d` byl řízeně nasazen; živý stav proudu Samantha Agent/RAG potvrdil `development=true`, `one_turn_write=true`, připojenou nečinnou relaci a smoke 5/5. Human–Adam a Knihovna zůstávají dočasnými kompatibilními adaptéry. | `tvbcp/architektura_komunikace_samantha.txt`; `technical/project_tvbcp_rules.md`; `technical/global_safety_brake.md`; `technical/capability_routing_rules.md` | Míla může v proudu Samantha Agent/RAG stisknout `Zahájit vývoj` a zadat první malý P7 krok; běžný tah bez tohoto povolení zůstává nezapisovací. |
