...
| R2-Adam / Janička | 2 | active | R2-Adam má vlastní trvalý chat, soukromý kontext, TXT prostor, dokumentovou lištu a čtečku. E2 živě ověřilo úplný tok e-mail -> vault -> create-only TXT. E3 potvrdilo ruční revizi a backendovou dostupnost PDF; lokální oprava odstraňuje diagnostickou obálku z lidského TXT bez změny staršího souboru. Nasazení opravy ještě chybí. | `projects/janicka_r2_adam.md` | `handoffs/workstreams/project-r2-adam-janicka.md`; `tvbcp/workstreams/project-r2-adam-janicka.md` | Po potvrzení nasadit opravu, obnovit Archiv e-mailu a R2 čtečku a ručně ověřit otevření PDF i čistý začátek TXT. |
...
| Cockpit hlavni architektura / modernizace | 1 | active | Cockpit Dieta D0-D3 je dokončená a nasazená: read-only mapa, oddělený backend Archivu e-mailu, frontend bez redesignu a rychlé doménové testovací sady. `app/cockpit.py` má při měření 30. 7. 2026 10 048 fyzických a 9 375 neprázdných řádků. | `../AuditCockpit56.txt`; `reports/cockpit_dieta_d0_2026_07_29.md`; `reports/cockpit_quality_gate_2026_07_10.md` | `handoffs/cockpit_architecture_current_2026_07_10.md` | Bez okamžitého dalšího dietního řezu. Pokračovat současnou roadmapou a další extrakci otevřít jen podle konkrétního problému. |
...
| iCloud Mail read-only / Email Cases | 1 | active | Read-only e-mailové workflow, Work Queue a Archiv e-mailu jsou funkční. Backend Archivu je oddělený z `cockpit.py`, frontend je ve statických souborech a oprava redigovaného dohledání archivních příloh je obsažená v nasazeném `main`. | `projects/email_readonly_oauth.md` | `handoffs/cockpit_email_archive_browser_2026_07_09.md` | Zopakovat read-only post-fix otevření známého redigovaného archivního záznamu a potvrdit dostupnost stažené přílohy. Zápisy, mazání a odesílání zůstávají mimo tento retest. |
