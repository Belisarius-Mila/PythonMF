...
| App-server rozhrani / novy Adam | 1 | active | Human–Adam a všechny zvolitelné proudy mají jednotný jednorázový vývoj. Server ukládá autoritativní stav dokončení, používá restartově odolnou idempotentní completion úlohu a čistý poradní tah lze označit jako dokončený bez změn místo falešného selhání. Všechny proudy smějí registrovaný read-only webový průzkum; rizikové private a destruktivní akce zůstávají za stávajícími hranicemi. Pojistky i poradní klasifikace jsou nasazené v commitu obsaženém v `91dc700`. | `tvbcp/architektura_komunikace_samantha.txt`; `technical/project_tvbcp_rules.md`; `technical/global_safety_brake.md`; `technical/capability_routing_rules.md` | `handoffs/human_adam_layer_workstream_start_2026_07_20.md` | Pokračovat běžným vývojem. Pokud se znovu objeví nedokončený tah, nejdřív porovnat serverový stav, completion job, Git a workspace; nevyvozovat stav jen z textu účtenky. |
...
| Vocabulary FR | 2 | paused | iPhone verze Míly i Jany mají funkční vzájemně výlučné filtry posledních 20/50 slovíček, opakované klepnutí filtr vypne a HT se kombinuje až nad vybraným posledním úsekem. Lokální macOS spuštění z Cockpitu používá opravený Tk runtime. | `projects/vocabularyfr_web_trainer.md`; `projects/pictnew_vocabulary_image_pipeline.md`; `tvbcp/workstreams/project-vocabulary-fr.md` | `handoffs/workstreams/project-vocabulary-fr.md` | Bez další změny. Při příštím zásahu aktualizovat Mílovu i Janinu určenou variantu společně a u Jany vždy ověřit `Sentence` a `SentenceT`. |
