...
| iPhone Shortcuts / Mobile Input Layer | 2 | active | Důležitá připomenutí mají dvě aktivní cesty: GitHub write-ahead do odděleného soukromého inboxu a přímý Tailscale POST. Fine-grained token je pouze v ignorovaném `.env`, podepsaná soukromá zkratka je nakonfigurovaná a Cockpit hotfix `20742e9` je nasazený. Živá kontrola potvrdila načtenou konfiguraci, GitHub sync bez chyb a Tailscale endpoint; skutečné iPhone testy ještě chybí. Quick Notes se tímto krokem neměnily. | `technical/iphone_shortcuts_playground.md`; `tvbcp/workstreams/project-cockpit.md` | `handoffs/iphone_shortcuts_freeze_infrastructure_layer_2026_05_25.md`; `handoffs/workstreams/project-cockpit.md` | Provést jedno neškodné připomenutí s bdícím Macem a potom test se spícím Macem; ověřit jednu lokální položku, bez duplicity, a GitHub Issue otevřenou jen do převzetí. |
