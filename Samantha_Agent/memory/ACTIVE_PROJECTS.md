...
| App-server rozhrani / novy Adam | 1 | active | Human–Adam, kanonické pracovní proudy a globální operace panelu Práce jsou funkční. Dne 2026-08-01 byl odstraněn historický allowlist tří zapisovatelných lazy proudů: všechny zvolitelné aktivní i pozastavené proudy nyní používají stejný jednorázový vývojový režim, zatímco běžný tah bez vývojového záměru zůstává nezapisovací a archivované proudy jsou mimo výběr. Oprava prošla 1263 testy, ale zatím nebyla nasazena do běžícího Cockpitu. Human–Adam a Knihovna zůstávají dočasnými kompatibilními adaptéry. | `tvbcp/architektura_komunikace_samantha.txt`; `technical/project_tvbcp_rules.md`; `technical/global_safety_brake.md`; `technical/capability_routing_rules.md` | `handoffs/human_adam_layer_workstream_start_2026_07_20.md` | Vytvořit lokální commit do denního GitHub balíčku; nasazení provést až samostatně potvrzeným krokem a potom živě ověřit tlačítko `Zahájit vývoj` v dříve nepovoleném proudu. |
