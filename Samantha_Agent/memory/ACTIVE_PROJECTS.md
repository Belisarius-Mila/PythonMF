...
| iPhone Shortcuts / Mobile Input Layer | 2 | active | GitHub write-ahead a přímý Tailscale POST jsou živé; backendová fail-closed ochrana je nasazená na `04e7763` a smoke prošel 5/5. Bdící běh varianty `3` vytvořil nový záznam 45, ale kvůli chybnému názvu Apple výstupu odeslal jen `delivery_id` `samantha-`, takže spánkový test zůstává pozastavený. Varianta `4` používá kanonické výstupy `Date` a `Formatted Date`, je validovaná a soukromě podepsaná, ale ještě není importovaná ani provozně otestovaná. Quick Notes se neměnily. | `technical/iphone_shortcuts_playground.md`; `tvbcp/workstreams/project-cockpit.md` | `handoffs/iphone_shortcuts_freeze_infrastructure_layer_2026_05_25.md`; `handoffs/workstreams/project-cockpit.md` | Importovat variantu `4`, provést jeden bdící test a ověřit celé nové časové `delivery_id`; teprve potom udělat jeden test se spícím Macem. |
