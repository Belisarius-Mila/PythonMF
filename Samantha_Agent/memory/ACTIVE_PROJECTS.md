...
| iCloud Mail read-only / Email Cases | 1 | active | Read-only e-mailové workflow a Work Queue jsou funkční. UX0+UX1 Archivu e-mailu je nasazené jako přehled ve stylu schránky. Následné UX ladění opravuje řazení podle skutečného data přijetí, bezpečný výběr úplnější textové alternativy z původního EML a přímé otevření celého originálního PDF místo mobilně problematického iframe. Oprava prošla plnou branou 1245/1245, ale zatím není nasazená. | `projects/email_readonly_oauth.md` | `handoffs/cockpit_email_archive_browser_2026_07_09.md` | Po samostatném potvrzení nasadit opravu, spustit smoke test a na iPhonu ověřit pořadí zpráv i otevření vícestránkového PDF. Zápisy, mazání a odesílání zůstávají mimo rozsah. |
