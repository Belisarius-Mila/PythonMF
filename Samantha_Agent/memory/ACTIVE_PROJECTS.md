...
| Znalostni databaze / Knihovna clanku / Knowledge inbox | 2 | active | Knihovna vedle článků a příloh obsahuje strukturované knihy: titul, autor, shrnutí, umístění, kategorie, obálku, ISBN a volitelný rok vydání. Umí přehled podle kategorií, autorů a umístění, dočasné OCR obálky a ISBN z fotografie. Katalogové dohledání porovnává obálku s Knihovny.cz a používá Open Library jako fallback; neshody se neukládají automaticky. Commit `91dc700` s rokem vydání je nasazený, navazující `5470644` s českým katalogovým porovnáním je otestovaný a čeká na samostatné nasazení. Soukromý obsah zůstává mimo Git. | `projects/vedecke_clanky.md`; `technical/large_context_intake.md`; `tvbcp/knihovna_cockpit.txt` | `handoffs/knowledge_library_article_editing_2026_07_16.md` | Nasadit aktuální katalogové porovnání a na iPhonu ověřit shodu, neshodu i obě ruční volby; uložení musí proběhnout až přes `Uložit úpravy`. |
...
| Cockpit Recovery centrum | 1 | active | Recovery, health, diagnostika, bezpečný restart, Tailscale přístup a pamatováček jsou funkční. Opakovaná řízená nasazení 2026-08-07 skončila novým procesem, serverovým kódovým otiskem a smoke kontrolou 5/5. Staré ruční retesty z června a července už nejsou startovní priorita. | `infrastructure/codex_reconnect_recovery.md`; `infrastructure/klicove_prikazy_pamatovacek.md`; `technical/session_recovery_rules.md` | `handoffs/cockpit_startup_health_voicebridge_verified_2026_07_09.md` (historický základ); `handoffs/workstreams/project-cockpit.md` | Bez okamžité akce. Při skutečném problému použít rychlý `/api/server/health`, Recovery centrum a až potom cílenou diagnostiku. |
| Cockpit hlavni architektura / modernizace | 1 | active | Cockpit Dieta D0-D3 a pravdivá navigace jsou nasazené. Přímé Tailscale doručení důležitých připomenutí i Quick Notes funguje, lokální VocabularyFR/VocabularyIT se spouštějí s použitelným Tk runtime a MultiLO je přidané do katalogu aplikací. Servisní souhrn dokumentového trezoru je zjednodušený. Ověřený běžící Cockpit obsahuje commit `91dc700`, serverový restart a smoke 5/5. | `../AuditCockpit56.txt`; `reports/cockpit_dieta_d0_2026_07_29.md`; `reports/cockpit_quality_gate_2026_07_10.md`; `tvbcp/workstreams/project-cockpit.md` | `handoffs/workstreams/project-cockpit.md`; `handoffs/cockpit_architecture_current_2026_07_10.md` (historická roadmapa) | Další informační přesuny otevírat jen podle konkrétní uživatelské zkušenosti. Nyní není doložený architektonický blokátor. |
| App-server rozhrani / novy Adam | 1 | active | Potvrzený Human–Adam checkpoint nově synchronizuje kanonický handoff, TVBCP a primární řádek proudu v `ACTIVE_PROJECTS.md` uvnitř jediného checkpointového commitu. Chybějící, duplicitní nebo katalogově rozporný řádek selže uzavřeně; chyba zápisu či commitu obnoví všechny tři soubory. Serverová completion úloha zůstává restartově odolná a autoritativní. Implementace prošla plnou branou 1329 testů, ale zatím není nasazená do běžícího Cockpitu. | `tvbcp/architektura_komunikace_samantha.txt`; `technical/project_tvbcp_rules.md`; `technical/global_safety_brake.md`; `technical/capability_routing_rules.md` | `handoffs/human_adam_layer_workstream_start_2026_07_20.md` | Po samostatně potvrzeném nasazení ověřit první přirozený checkpoint a následný systémový audit s nulovým driftem. |
...
| ToBeToHave | 2 | active | Kanonická desktopová výuková aplikace je `ToBeTraining/tobe_trenink.py`. Procvičuje to be, to have a to go ze dvou stávajících lokálních CSV, která se při Cockpit integraci nezměnila. Proud `project-to-be-to-have` i allowlistovaná karta v nabídce Webových aplikací jsou nasazené; server potvrdil nový proces, smoke 5/5, živý katalog i skutečný start aplikace přes Python 3.12/Tk 8.6. | `projects/to_be_to_have.md`; `WORKSTREAMS.md` | zatím nematerializováno; stabilní lazy cesta `handoffs/workstreams/project-to-be-to-have.md` | Bez okamžité změny. Další úpravy aplikace nebo výukových CSV dělat jen na samostatné zadání; při provozním tvrzení znovu ověřit živý stav. |
