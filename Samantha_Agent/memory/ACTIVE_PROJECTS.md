...
| iPhone Shortcuts / Mobile Input Layer | 2 | active | GitHub write-ahead a přímý Tailscale POST jsou živé. Bdící test prošel, ale spánkový test odhalil opakované `delivery_id`: nový text zůstal jen v uzavřené testovací Issue a lokálně se nevytvořil. Varianta `3` proto používá čerstvé milisekundové časové ID a backend má připravenou fail-closed ochranu, která při stejném ID s jiným textem Issue nezavře. Backendová ochrana ještě není nasazená a varianta `3` není importovaná. Quick Notes se neměnily. | `technical/iphone_shortcuts_playground.md`; `tvbcp/workstreams/project-cockpit.md` | `handoffs/iphone_shortcuts_freeze_infrastructure_layer_2026_05_25.md`; `handoffs/workstreams/project-cockpit.md` | Dokončit checkpoint, samostatně potvrdit nasazení backendové pojistky, importovat variantu `3` a ověřit dvě různá ID před novým testem nedostupného Macu. |
