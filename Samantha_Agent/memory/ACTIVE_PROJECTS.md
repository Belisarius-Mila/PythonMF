...
| iPhone Shortcuts / Mobile Input Layer | 2 | active | Důležitá připomenutí mají v ověřeném lokálním checkpointu dvě aktivní cesty: přímý Tailscale POST do Cockpitu a write-ahead Issue v odděleném soukromém GitHub inboxu. Cockpit už pro připomenutí nesynchronizuje iCloud. Syntetický GitHub pilot bez soukromého textu prošel; podepsaná zkratka je připravená mimo git. Produkční token, import na iPhone a nasazení Cockpitu zatím chybí. Quick Notes se tímto krokem neměnily. | `technical/iphone_shortcuts_playground.md`; `tvbcp/workstreams/project-cockpit.md` | `handoffs/iphone_shortcuts_freeze_infrastructure_layer_2026_05_25.md`; `handoffs/workstreams/project-cockpit.md` | Vytvořit fine-grained token omezený na soukromý inbox s oprávněním Issues read/write, vložit jej pouze do lokálního `.env` a při importu zkratky; potom samostatně nasadit Cockpit a provést jeden skutečný iPhone test s bdícím a jeden se spícím Macem. |
