...
| Linux / instalace a konfigurace | 2 | active | Nový konzultační projekt pro počítač s Pentiem G4560, 8 GB RAM a novým 480GB SATA SSD. První bezpečné rozhodnutí je instalovat Linux na nový SSD a původní HDD s Windows zatím ponechat beze změny jako návratovou cestu. | `projects/linux_workstation.md`; `WORKSTREAMS.md` | zatím nematerializováno; stabilní lazy cesty `handoffs/workstreams/project-linux-workstation.md`; `tvbcp/workstreams/project-linux-workstation.md` | Po dodání SSD připravit a ověřit živé USB; při instalaci odpojit původní HDD a nejdřív otestovat hardware bez mazání Windows. |
