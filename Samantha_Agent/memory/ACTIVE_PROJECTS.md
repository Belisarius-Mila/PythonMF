...
| Samantha Agent/RAG | 1 | active | P0 zavedlo read-only audit pravdivosti 30 pracovnich proudu. P1 pridalo autoritu zdroju `canonical`, `aggregate`, `aggregate_unverified`, `reference` a `historical`. P2 zalozilo kanonickou dvojici tohoto proudu a P3 opravilo jediny formalni rozpor Mobile Input. P4 prokazalo, ze textovy RAG radi spravne, ale agent pri aktualnim stavu chybne vnucoval filtr `projects`. P5a zavedlo prvni hledani bez filtru a zastaveni po relevantnim `canonical`; P5b pridalo pouze jednoznacne query aliasy `R2 Adam` a `Kalendář`. Cilenych 47 a uplnych 1240 testu proslo, stejne jako live dotazy obou aliasu. | `samantha_core.md`; `tvbcp/workstreams/project-samantha-agent-rag.md` | `handoffs/workstreams/project-samantha-agent-rag.md` | Potvrzene nasadit P5 do Cockpitu a provest smoke test; embeddings zatim nepridavat. |
