...
- Aktualizováno: 2026-09-25 21:52 CEST — lokální M2a.

### M2a — nejnovější stav

- Míla potvrdil, že syntetický M1 náhled funguje. Bez dodatečného tvrzení
  o konkrétním prohlížeči, jednotlivých médiích či vzdálené přejímce.
- Místní M2a: volitelný runtime Vieweru, sériový 60s worker, soukromý URL
  prefix a konfigurovaný odkaz z Cockpitu. Testy a hranice:
  `camino/docs/M2_RUNTIME_REPORT.md`.
- Žádné nasazení, vydání skutečného tokenu, povolení cesty, změna iPhonu,
  launchd/Serve/Funnel ani zpřístupnění osobních dat neproběhly.
- Další krok M2b: minimální přenos pořadí a mezer audiočástí pro Viewer.
  Potom registrované provozní ovládání a samostatně schválené nasazení.
- M2 není dokončené; M3 záloha a M4 přejímka zůstávají, AI odložena.
...
- M1 náhled je potvrzený a M2a připravilo místní runtime/odkaz. Další je M2b
  pořadí audia, potom dokončení privátního provozu M2, M3 minimální nezávislá
  záloha a M4 jeden společný předcestovní průchod.
...
- T048 prošel na fyzickém iPhonu v dostupném syntetickém rozsahu cizí Wi-Fi. Běžný přenos skončil jedinou ověřenou relací 13/13. Při vypnutí Wi-Fi klient i server současně zůstaly na 2/13 bez objektu a po návratu se bez dalšího stisku doplnily jen chybějící části. S vypnutým tailnetem nová dávka nevytvořila serverovou relaci ani veřejný fallback; po obnovení Tailscale se bez dalšího stisku dokončila. Všechny tři dávky skončily na telefonu `Ověřeno na Macu` a server má 3 relace, 3 objekty a 3 účtenky, každý 100 663 553 B se shodným SHA-256. Captive portal zůstává NEOVĚŘENO, protože použitá síť jej neměla.
- T048 je registrovaně ukončený: `phase=stopped`, receiver i vlastní Serve cesta jsou neaktivní, původní Serve je přesně obnovený a Funnel vypnutý. Tři syntetické důkazy zůstávají zachované mimo Git.
...
- T049 bylo registrovaně ukončeno: vlastní receiver neběží, testovací Serve cesta je odebraná, původní konfigurace přesně obnovená, Funnel vypnutý. Jediný ověřený syntetický důkaz zůstává zachovaný.
...
- Build 0.4.0 (4) je místně podepsaný a nainstalovaný na iPhonu 14 Plus přes stejné bundle ID. Přísný podpis, platný profil zahrnující telefon a seznam nainstalovaných aplikací s verzí 4 prošly. Camino Audio zůstává nainstalované, starší podepsaný build 3 je na Macu zachovaný. Cílený postup A/B a výsledek jsou v `camino/tasks/C02b_BUILD4_FIRST_START_FIELD_PLAN.md`.
- Fyzický build 4: část A PASS (po mobilním grantu, návratu z Ovládacího centra a obnově sítě telefon 0/13, server 0 relací). Část B prokázala vědomý start, přerušení při 1/13 bez finálního objektu a automatické dokončení po nuceném ukončení a otevření aplikace s dostupnou sítí. Telefon hlásil `Ověřeno na Macu`; server 13/13, jednu ověřenou relaci, objekt a účtenku, 100 663 553 B a shodný SHA-256. Otevření aplikace ještě bez sítě neproběhlo, takže celý plán B není PASS.
...
- C04d je hotové: deník umí výběr dne, filtry, detail Momentu, trvalý koncept textu po pádu, lidskou i automatickou historii revizí s předností lidské úpravy, zamknutí, skrytí a obnovu, přesun kapitoly bez změny původního času a samostatný soukromý doplněk navázaný na původní Moment. Úvahu lze uvolnit pouze výslovnou akcí `Vložit do deníku`; změny pro budoucí C05 mají trvalé pořadí a stav čekání na server. Schéma Core Data se nezměnilo: rozšíření je verzované v existujícím `SettingRecord`, aby se zachovalo otevření C04b databáze. Swift 22/22, UI simulátoru 4/4, nepodepsaný generic iOS build a plná brána 1740/1740 PASS. Zdroj `e3868100` byl pushnut na `origin/main`; podepsané Camino 0.1.0 (2) prošlo strict kontrolou, instalací a spuštěním na iPhonu 14 Plus / iOS 26.6.1 bez odinstalace a bez ztráty starších C04b dat.
- C05a má lokální produkčně orientovaný serverový checkpoint. FastAPI přijímá streamované části pouze podle přijatého C03b manifestu, server počítá hash části i celého objektu a dvojitý SQLite/souborový journal obnovuje pád mezi zápisy. Shodné retry je idempotentní, konflikt nic nepřepíše, neznámé bajty se zachovají v karanténě a poškozený hotový objekt zruší `verified`. Owner token je odvolatelný a uložený jen jako hash; proces povoluje pouze loopback. Registrovaný smoke `20260923T205907Z-e64b0346` prošel 12/12 kontrolami přes skutečné loopback HTTP, 1 048 699 B v 5 částech, včetně retry, finalizace, odvolání obou tokenů a restartu se zachovaným `verified`. Smoke workflow 4/4, společná cílená sada 30/30 a plná brána 1763/1763 PASS.
...
- C05c je implementované a zdroj `f0f714f4` pushnutý. Obrazovka **Uložení a přenosy** má čtyři nezávislé osy Telefon, Mac, Další záloha a AI; Momenty se nepletou s počtem operací, médií nebo bajtů. Podepsané Camino 0.1.0 (4) prošlo strict podpisem, shodou týmu a profilem zahrnujícím iPhone a bylo nainstalováno bez odinstalace a spuštěno. T054/T055 mají pouze syntetický důkaz a fyzický průchod C05c čeká.

### Otevřeno
- Otevření staršího C02b buildu 4 bez sítě po nuceném ukončení zůstává historicky neověřené; integrované C05b T047-B však fyzicky prošlo. U T048 zůstává `NEOVĚŘENO` pouze captive portal.
- C05b je uzavřené v akceptačním session-owned rozsahu. C05c je pushnuté a nainstalované, ale čeká na krátký fyzický průchod stavového UX a potvrzení zachování dat. Skutečné konce T054/T055 čekají na C06b/C07. Viewer, T038, serverová větev Důležité a C06a zůstávají otevřené.

### Rizika
- C05a/C05b byly fyzicky použity jen jako session-owned akceptační služba. Nejde o trvalý proces, provozní dohled ani zálohu a přenesené bajty nejsou měřením spotřeby operátora.
- Samostatný C02b harness bez kamery a jeho neúplná část B zůstávají historickými omezeními prototypu; plné integrované C05b T047-B a T049 už fyzicky prošly.
...
- C05a loopback smoke ověřil síťový adaptér a bajty Assetu nad syntetickými daty. Není to spravovaný proces, Tailscale Serve, produkční úložiště ani záloha. Párovací UI patří C06 a skutečná záloha/obnova C06b. Obnovovací porovnání C03b je zatím jen pro Momenty.
- C05b fyzický průchod dokládá zámek, force quit, síťový přechod, mobilní dávku a pozorované UX na tomto iPhonu. Akceptační server však zůstává session-owned, ne trvalý C06a provoz ani druhá kopie dat.
- C05c odděluje tvrzení v UI, ale samo nevytváří druhou kopii ani AI výsledek. Zelená syntetická osa v unit/UI testu se nesmí vydávat za fyzický nebo produkční PASS.
- Integrovaná aplikace včetně low-space/thermal hardeningu je podepsaná a nainstalovaná; starší C04b i nové C04d lokální scénáře uvedené výše fyzicky prošly. T007 a plné T060 zůstávají částečné bez skutečného nízkého místa; simulovaná kapacita ani teplota nejsou fyzická akceptace. Nový bundle ID nepřebírá prototypová audio data; místní úložiště není záloha na Macu.
- Operace označené `čeká na server` jsou trvalý lokální frontový podklad, ne důkaz synchronizace ani serverového přijetí. C04d fyzicky ověřilo mikrofon, hovory, změnu audio vstupu, nucené ukončení aplikace, přehrávání a skutečné UX; neověřilo produkční server, Viewer ani opožděnou AI větev T038.

### Další krok
- Dne 2026-09-25 projít `C05c_IPHONE_TEST_PLAN.md`, počínaje zachováním dat a čtyřmi sekcemi. C06a/C06b/C07 se tím neotevírají.
...
- Míla následně výslovně povolil celý navazující rozsah až do C05b včetně push, řízeného nasazení a podepsané aktualizace iPhonu; Funnel je výslovně mimo rozsah. Registrované síťové mutace nadále vyžadují svůj náhled a samostatné `ano`.
- Míla zahájil C05c. Tento krok implementuje pouze pravdivé stavové UX a automatický důkaz; předchozí oprávnění k pushi, nasazení a instalaci pro C05b se na nový checkpoint nepřenáší.
- Míla následně výslovně povolil push a podepsanou aktualizaci C05c; fyzický průchod odložil na 2026-09-25. Povolení nezahrnuje nasazení Cockpitu, start C05b serveru, Serve ani Funnel.

### Navrhované další kroky
- T047 je fyzicky PASS v syntetickém rozsahu; jeho dva objekty/účtenky a relace zůstávají zachované.
- Integrované T049 s novým skutečným videem je PASS; starší syntetický harness zůstává jen historickým důkazem.
...
- Dne 2026-09-25 fyzicky ověřit zachování dat, čtyři sekce a nezávislost místního záznamu podle C05c plánu. Serverové větve T040/T096 a produkční AI větev T038 zůstávají otevřené. C05b Serve znovu spouštět jen po novém náhledu a potvrzení.
...
- T050 před fyzickým testem: cílené testy 32/32 a plná projektová brána 1719/1719 PASS; kód se při testu neměnil. Server `verifying` 17:58:22–17:58:36 CEST při 13/13 a 0 objektech/účtenkách; soukromé video ukazuje `Ověřuji` v tomto okně a finální stav až po něm. Poslední T050 status po stopu: `phase=stopped`, `receiver_alive=false`, `private_route_exact=false`, `funnel_enabled=false`, `session_count=1`, `session_verified_count=1`, `object_count=1`, `receipt_count=1`, `verified_match=true`, `verified_byte_count=100663553`. V době T050 byl T048 `INACTIVE` a T049 `stopped`; push ani nasazení neproběhly.
- T048 audit po stopu: `phase=stopped`, `receiver_alive=false`, `private_route_exact=false`, `funnel_enabled=false`, `session_count=3`, `session_verified_count=3`, `session_incomplete_count=0`, `latest_session_chunks=13/13`, `object_count=3`, `receipt_count=3`, `verified_match=true`, `verified_byte_count=301990659`. Původní Serve byl přesně obnoven; kód se při fyzickém testu neměnil.
- Oprava buildu 4: Swift 22/22 včetně nového i staršího journalu, simulátorový UI test exit 0, nepodepsaný generic iOS build exit 0 a plná projektová brána 1719/1719 PASS. Podepsaný iOS build, strict podpis a CoreDevice instalace verze 4 PASS. Nový T049 audit po stopu: `phase=stopped`, `receiver_alive=false`, `private_route_exact=false`, `funnel_enabled=false`, `session_count=1`, `session_verified_count=1`, `latest_session_chunks=13/13`, `object_count=1`, `receipt_count=1`, `verified_match=true`, `verified_byte_count=100663553`. Kód se při fyzickém testu neměnil.
...
Datum: 2026-09-20 08:50 CEST

Co se resilo:
C03b — verzované owner API a trvalé revize.

Co je hotove:
Referenční JSON API v1, OpenAPI a soukromé SQLite úložiště ukládají přijímané metadata, textové revize, účtenky a konflikty. Souvislé pořadí, přesný retry, nový epoch po řízené obnově a blok výstupů jsou pokryté syntetickými testy 12/12. Dřívější C02b fyzický stav a potvrzený stop T049 zůstávají dle aktuálního bloku výše.

Co neni hotove:
C03b nemá síťový listener, produkční autentizaci, skutečná média ani iPhone/Viewer integraci; plné T044/T045/T053/T058, U15 T096 a G0/G1/G8 zůstávají otevřené. C02b: offline relaunch buildu 4 po force quit není ověřený, celý plán B není PASS, T048 je neprovedené a plné T049 čeká na skutečné video. C05a/C06b musí řešit produkční server, média a obnovu; plný T061 chybí.

Dalsi krok:
Po samostatném zadání C04a místní úložiště a hlavní obrazovka; T048 odděleně při dostupné cizí Wi-Fi podle připraveného postupu.

Navrhovane dalsi kroky:
C05a naváže produkčním privátním serverem a ověřenými médii, C06b skutečnou obnovou; C08 vynutí Viewer práva. Plné T049 s novým skutečným videem až v integrované aplikaci.

Zmenene nebo relevantni soubory:
`camino/api/`, `camino/domain/`, `camino/docs/C03b_API_CONTRACT.md`, `C03b_OPENAPI_V1.json`, `tests/test_camino_c03b_contract.py`, brána, projektová paměť a kanonický TVBCP.
...
### 2026-09-19 21:35 CEST — Podpis, instalace a plán cílené zkoušky

Hotovo:
- Build 0.4.0 (4) byl podepsán a jako aktualizace instalován na iPhone 14
  Plus. Předchozí podepsaný build 3 zůstal na Macu, Camino Audio v telefonu.
- Nový plán A/B stanoví přesné mezistavy telefonu a serveru před vědomým
  startem a po obnově již zahájeného přenosu.

Rozhodnutí:
- Produktové chování se neměnilo; uživatelský pokyn zahrnoval podpis,
  instalaci a přípravu testu. Soukromý receiver se zatím nespustil.

Další krok:
- Po samostatném pokynu spustit registrovaný T049 běh a na nové syntetické
  dávce podle `C02b_BUILD4_FIRST_START_FIELD_PLAN.md` fyzicky provést A/B.

Navrhované další kroky:
- Po testu registrovaně ukončit běh a doložit obnovu Serve; T048 nechat na
  dobu, kdy bude dostupná cizí Wi-Fi.

Technický důkaz:
- Podepsaný build a strict podpis PASS, platný profil zahrnuje připojený
  iPhone, CoreDevice potvrzuje nainstalovanou verzi 4. T049/T050 jsou zastavené,
  receiver/cesta neaktivní, Funnel vypnutý. Launch a fyzický přenos NEPROVEDENO.

### 2026-09-19 22:11 CEST — Build 4 fyzicky ověřen v dostupném rozsahu; T049 zastavené

Hotovo:
- Mobilní grant, návrat z Ovládacího centra a obnovení sítě ponechaly novou
  dávku na 0/13 bez serverové relace: část A PASS.
- Vědomý start vytvořil jedinou relaci. Po síťovém přerušení při 1/13 a později
  8/13 se aplikace nuceně ukončila; po otevření s dostupnou sítí pokračovala bez
  dalšího stisku do 13/13. Telefon ukázal `Ověřeno na Macu`, server potvrdil
  jediný objekt a účtenku se shodnou délkou a SHA-256.
- Potvrzený stop odebral vlastní trasu/receiver, přesně obnovil původní Serve
  a zachoval syntetický důkaz; Funnel zůstal vypnutý.

Rozhodnutí:
- Otevření aplikace ještě bez sítě neproběhlo, proto celý plán B není PASS.
  Míla zvolil neopakovat dávku jen kvůli tomuto mezikroku: Letový režim zároveň
  odpojuje chat běžící přes mobilní spojení iPhonu. Nový vývoj není potřeba.

Další krok:
- T048 provést při dostupné cizí Wi-Fi podle připraveného postupu, s novým
  potvrzeným receiverem a syntetickou dávkou.

Navrhované další kroky:
- Plné T049 se skutečně novým videem až v integrované aplikaci. Offline
  znovuotevření buildu 4 zůstává mezera důkazu; T050 bez nové pochybnosti
  neopakovat.

Technický důkaz:
- Audit po stopu: `phase=stopped`, `receiver_alive=false`,
  `private_route_exact=false`, `funnel_enabled=false`, jedna ověřená relace,
  objekt a účtenka, 13/13, 100 663 553 B, `verified_match=true`.

### 2026-09-19 23:25 CEST — C03a datový kontrakt a soukromí

Hotovo:
- Přidán čistý model Trip/Day/Moment/Asset, append-only metadata a textových
  revizí, časové provenience a volitelné polohy. U15 vynucuje nový soukromý
  Moment Úvahy, explicitní revizi pro vložení do deníku a fail-closed výběr
  Viewer kandidátů jen z poslední serverem přijaté revize.

Rozhodnutí:
- C03a je referenční doménový kontrakt bez migrace C01/C02 prototypů. Nejde o
  provozní iPhone/server/Viewer integraci ani fyzický PASS akceptačních testů.

Další krok:
- C03b: verzované API, serializace a mapování do persistence včetně konfliktů.

Navrhované další kroky:
- C04/C05/C08 vynutit U15 a provést fyzické T096. T048 samostatně při cizí
  Wi-Fi; plné T049 až s novým skutečným videem v integrované aplikaci.

Technický důkaz:
- `tests.test_camino_domain` 9/9, plná projektová brána 1728/1728 PASS.
  Žádný push, instalace iPhone ani nasazení v C03a.

### 2026-09-20 08:52 CEST — C03b verzované API a trvalé revize

Hotovo:
- Lokální owner kontrakt `/api/v1/` přijímá přesné JSON objekty C03a a vrací
  stabilní chyby, stav epochy a souvislý kurzor. SQLite uchovává metadata,
  manifesty, textové i metadatové revize, původní operace a účtenky po restartu.
- Shodný retry nezdvojí zápis; změněné bajty nebo kolize revize zůstanou jako
  konfliktní kandidát a zablokují výstupy. Obnova změní epochu a porovná
  inventář Momentů bez automatického odblokování.

Rozhodnutí:
- Míla zadal vyzkoušet verzované API a ukládání revizí. C03b je lokální
  referenční owner kontrakt bez síťové služby a bez migrace C01/C02.

Další krok:
- Po samostatném zadání C04a připojit místní úložiště a hlavní obrazovku
  iPhonu ke stabilním ID a revizím.

Navrhované další kroky:
- C05a napojí produkční privátní HTTPS, ověření zařízení, skutečná média a
  řízené řešení konfliktů; C06b skutečnou obnovu a inventář Assetů.
- T048 provést samostatně na cizí Wi-Fi; plné T049 a T053/T058 až v integrované
  aplikaci. C08 musí vynutit aktuální soukromí při každém vydání média.
...
Hotovo:
- Po dvou rychlých fotografiích zůstaly dva samostatné Momenty. První má
  výslovně přidanou další fotografii a komentář, druhý jen svou fotografii;
  Míla potvrdil vazby i po úplném zavření a otevření aplikace.

Rozhodnutí:
- T008 PASS na fyzickém iPhonu 14 Plus podle Mílova pozorování. T007 je
  stále částečný kvůli nevyvolané chybě zápisu.

Další krok:
- T011 skutečné krátké video se zvukem: Stop, restart, jedna položka,
  přehrání obrazu i zvuku a přiměřená délka.

Navrhované další kroky:
- T012–T014, T009 a bezpečná část T060; pak C04d/C05a.
...
- Na fyzickém iPhonu a skutečně jiné Wi-Fi prošel běžný soukromý přenos jedné
  nové syntetické dávky. Telefon i server souběžně doložily postup 1/13 až
  13/13, následnou finalizaci a stav `Ověřeno na Macu`.
- Druhá dávka byla při 2/13 přerušena vypnutím Wi-Fi. Klient zůstal na 17 %,
  2/13 a server po dobu kontroly na stejné relaci 2/13 bez objektu či účtenky.
  Po návratu stejné Wi-Fi se bez dalšího stisku doplnily jen chybějící části a
  vznikl právě jeden další ověřený objekt a účtenka.
- Třetí dávka s vypnutým tailnetem zůstala ve výchozím stavu a server nevytvořil
  žádnou novou relaci; veřejná náhradní cesta nevznikla. Po obnovení Tailscale
  se bez dalšího stisku vytvořila jediná relace a doběhla do ověřeného finále.
- Registrovaný stop odebral pouze vlastní `/camino-c02b`, ukončil vlastněný
  receiver, přesně obnovil původní Serve a ponechal Funnel vypnutý.

Rozhodnutí:
- T048 má PASS v dostupném syntetickém rozsahu běžné cizí Wi-Fi, přechodu
  povolené/nedostupné sítě a nedostupného/obnoveného tailnetu. Captive portal
  zůstává NEOVĚŘENO, protože použitá síť jej neměla; celý kanonický T048 se
  proto neoznačuje bez omezení za úplný PASS.
- Jde o syntetický receiver a testovací aplikaci, nikoli produkční server,
  zálohu skutečných médií nebo akceptaci Vieweru.

Další krok:
- Navázat v Human–Adam dokončením T013 s odepřeným mikrofonem, potom T014,
  T009 a bezpečnou částí T060; následně C04d a C05a.

Navrhované další kroky:
- Captive portal T048 doplnit pouze při přirozeně dostupné síti. Tři již
  doložené podscénáře neopakovat bez nové pochybnosti.
- Plné T049 provést až s novým skutečným videem a produkčním propojením.

Technický důkaz:
- Finální audit T048: `phase=stopped`, receiver/cesta neaktivní, Funnel
...
Navrhované další kroky:
- Po T009 provést bezpečnou část T060; potom C04d a C05a.
...
Navrhované další kroky:
- Po technickém hardeningu C04b pokračovat C04d a C05a.
...
Hotovo:
- FastAPI adaptér spojil C03b metadata s obnovitelnými částmi a ověřenou
  finalizací. Přijatý Asset manifest je autorita a stav `verified` vyžaduje
  serverový hash celého objektu i trvalou účtenku.
- Odvolatelné owner tokeny se zakládají mimo síťové API a ukládají jen jako
  hash. Listener odmítá vše kromě loopbacku; neznámé bajty se nemažou.

Rozhodnutí a rizika:
- T043–T046/T052/T061 mají lokální serverový syntetický důkaz, nikoli nový
  fyzický nebo produkční PASS. Služba, privátní cesty/token, Tailscale Serve,
  C05b klient, push a nasazení nebyly provedeny.

Další krok:
- Samostatně připravit privátní cesty/token a loopback smoke nad prázdnými
  syntetickými daty; potom zvlášť rozhodnout o Serve a C05b.
...
Hotovo:
- Camino 0.1.0 (3) bylo aktualizováno bez odinstalace a zachovalo starší data.
  Míla fyzicky potvrdil T051, T047-A/B, dostupnou část T048, T049, T050, T052,
  T053, T058 a trvalé pozastavení.
...
Rozhodnutí:
- C05b je přijato v session-owned fyzickém rozsahu. Captive portal T048 je
  `NEOVĚŘENO`; C06a/C06b ani Viewer se tím neuzavírají.

Další krok:
- Navázat C05c stavovým UX.

Navrhované další kroky:
- C06a a C06b zahájit jen samostatným rozhodnutím; captive portal doplnit jen
  při bezpečně dostupné síti a T038 až s reálnou opožděnou AI.
