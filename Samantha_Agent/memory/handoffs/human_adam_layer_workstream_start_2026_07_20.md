...
- Obnoveno potvrzeným checkpointem: 2026-07-27 01:21 CEST

### Hotovo
- Předchozí `main` (`28885b957d34`) je zarovnaný s GitHubem, serverově nasazený
  a Human–Adam je připojený.
- Redigovaný status nově vydává pouze bezpečný 12znakový `source_head_short`;
  celý Git hash ani private metadata nezveřejňuje.
- Stavový banner porovnává tento bezpečný otisk s deployment receipt a při shodě
  jednoznačně potvrdí společný commit `Git/main` a běžícího Cockpitu.
- Neplatný nebo chybějící otisk zůstává fail-closed a nesmí být vydáván za shodu.

### Otevřeno
- Tento nový checkpoint ještě nemá vlastní serverový důkaz nasazení.
- Zbývá jediné potvrzené nasazení a vizuální kontrola, že banner po restartu
  ukazuje shodu a už nehlásí `aktuální Git/main nelze ověřit`.

### Rizika
- Zkrácený otisk slouží pouze k provoznímu porovnání; autoritativní důkaz
  nasazení nadále drží serverová receipt.
- Chybějící nebo neplatný otisk záměrně ponechá stav jako neověřený.

### Další krok
- Provést read-only audit nového `main`, jednou jej potvrzeně nasadit do
  Cockpitu a vizuálně ověřit zelený banner se společným krátkým commitem.

### Rozhodnutí
- Veřejný status nebude kvůli banneru rozšiřován o celý Git hash.
- Jediným novým kontraktem je allowlistovaný 12znakový otisk; při neplatném
  formátu musí porovnání selhat uzavřeně.

### Navrhované další kroky
- Po živém ověření se vrátit k fázi 9.3e oddělení ochrany relací Janičky a
  diagnostických skriptů od Voice Bridge markeru.
- Samostatné Voice Bridge moduly a private stav řešit až v pozdější auditované fázi 9.4.

### Technický stav checkpointu
- Změna je otestovaná (83 úzkých, 338 širších a 1252 úplných testů).
- Python, oba JavaScripty, shell, whitespace a Git safety kontroly prošly.
- Git před checkpointem: `main == origin/main` na `28885b957d34`.
- Poslední serverově potvrzené nasazení: `28885b957d34` · 1251 testů · smoke 5/5.
