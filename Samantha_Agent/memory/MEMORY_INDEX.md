...
- `handoffs/human_adam_project_bootstrap_phase0_wip_2026_07_19.md` - [PRIPOMENOUT]
  priorita 1: v izolovane vetvi je hotova faze 0 pro zalozeni dosud chybejiciho
  projektu a prvniho handoffu primo z `Human-Adam -> Prace`. Nejdriv ukaze
  read-only nahled, zapis vyzaduje presnou vetu, meni jen profilovy workspace a
  pripne stejny semafor; sama nedela commit, push ani nasazeni. Napoveda pokryva
  fazi 0 az 4. Cilenych 106 testu i plna Cockpit brana s 862 testy prosly;
  zbyva prevzeti do `main`, restart, smoke test a rucni profilovy proklik.
...
- `handoffs/adam_voice_bridge_end_to_end_checkpoint_2026_06_05.md` - historicky Adam Voice Bridge end-to-end checkpoint; prekryto aktualnim provoznim kontraktem `handoffs/voicebridge_operational_contract_2026_06_30.md`.
- `handoffs/adam_voice_bridge_iphone_text_fallback_checkpoint_2026_06_06.md` - historicky Adam Voice Bridge / iPhone text fallback checkpoint; prekryto aktualnim provoznim kontraktem `handoffs/voicebridge_operational_contract_2026_06_30.md`.
- `handoffs/adam_voice_remote_cockpit_next_step_2026_06_06.md` - historicky Adam Voice Remote Cockpit smer k iPhone prehrani a approval centru; prekryto aktualnim provoznim kontraktem a Cockpit confirmation kartami.
- `handoffs/adam_voice_cockpit_readonly_capabilities_next_2026_06_06.md` - historicky Adam Voice / Cockpit checkpoint k read-only capability registry; prekryto dokoncenym capability registry a aktualnim provoznim kontraktem VoiceBridge.
- `handoffs/adam_voice_ttys001_readiness_blocker_2026_06_07.md` - historicky Adam Voice readiness blocker pro `ttys001`; prekryto pozdejsim TTY switcherem, managed bridge a provoznim kontraktem VoiceBridge.
...
- `handoffs/adam_voice_bridge_tty_switcher_2026_06_10.md` - historicky Adam Voice Bridge / Cockpit TTY prepinac; prekryto pozdejsim managed bridge stavem a provoznim kontraktem VoiceBridge.
- `handoffs/adam_voice_iphone_audio_bridge_checkpoint_2026_06_11.md` - historicky Adam Voice iPhone audio bridge checkpoint; prekryto potvrzenym iPhone autoread stavem a provoznim kontraktem VoiceBridge.
- `handoffs/adam_voice_bridge_cleanup_and_cockpit_controls_2026_06_12.md` - historicky Adam Voice Bridge cleanup; prekryto aktualnim provoznim kontraktem VoiceBridge a Cockpit confirmation kartami.
- `handoffs/adam_voice_cockpit_dev_runner_checkpoint_2026_06_12.md` - historicky Adam Voice / Cockpit Dev runner checkpoint; servisni runner a confirmation karta jsou zahrnute v aktualnim Cockpit/VoiceBridge stavu.
- `handoffs/adam_voice_bridge_freeze_email_return_2026_06_12.md` - historicky Adam Voice Bridge freeze pred navratem k e-mailum; prekryto VoiceBridge full-access uzavrenim a provoznim kontraktem 2026-06-30.
...
- `handoffs/janicka_light_samantha_bridge_checkpoint_2026_07_03.md` - [PRIPOMENOUT] Janička light Samantha bridge: po problému s druhým dotazem vznikla samostatná light relace `samantha_janicka`, ověření screen doručení přes `Request ID`, TTY fallback a read-only `codex exec` fallback s opravenou PATH pro `node`; další krok je ruční retest více navazujících dotazů v Cockpitu.
- `handoffs/janicka_chat_ui_fallback_simplification_2026_07_03.md` - Janička chat UI cleanup: hlavní viditelná cesta je `Janička chat` / `Spustit Janičku`, starý `samantha_adam` je schovaný pod `Servisní fallback`, pending dotazy jsou označené jako staré nevyřízené fallback dotazy.
