...
- `WORKSTREAMS.md` - kanonicky registr 32 pracovnich proudu: 26 projektu, 4 tooly a 2 `Misc`. Faze 4.2 pridala lazy soukroma vlakna, faze 4.3 jejich handoff/TVBCP vazby, faze 4.5e jednotny backendovy registr a faze 4.5f jednu perzistentni autoritu `active_workstream_id` ve schematu 2. Human–Adam a Knihovna jsou docasne kompatibilni adaptery nad svymi puvodnimi session a workspaces; ostatnich 30 proudu pouziva lazy private-thread backend. Faze 4.5g-d2 doplnila chybejici samostatny proud Rodinny kalendar s jednorazovou direct-main autorizaci; 2026-08-10 přibyl aktivní proud ToBeToHave a 2026-08-22 samostatný proud Linux.
...
- `reports/cockpit_ui_u06_2026_09_14.json` - U06 navigace a 16 dialogů, zachované drafty při návratu; 28 cílených testů a tři browser scénáře včetně vnořených návratů a šesti hash odkazů. Lokálně bez nasazení.
- `reports/cockpit_ui_u05_2026_09_14.json` - U05 jedna prioritní fronta, práce oddělená od alarmů; 24 cílených testů, 10 browser scénářů a statická brána OK. Lokálně bez nasazení.
...
- `reports/cockpit_ui_release_2026_09_14.md` - souhrn U01–U06, funkční nasazení 980aed7a, smoke 5/5 a brána 1680/1680 OK; publikační překážka odstraněna, U07 až příště.
- `reports/cockpit_ui_audit_2026_09_14.json` - evidence hlubokého UI auditu: šest syntetických browser scénářů, rozměry a navigační vady, inventura prvků, dva kontrakty a čerstvý redigovaný live audit. Hlavní text/plán na začátku AuditCockpit56_2.txt.
...
- `../AuditCockpit56_2.txt` - UI a strukturální audit v2.8: 22 výchozích nálezů, 11 kroků; U01–U06 funkčně nasazené a ověřené, uzavírací zápis součást schváleného GitHub balíčku. U07 až příště. Historický audit zachovaný níže.
...
- `reports/systemovy_audit_projekty_tooly_vrstvy_2026_08_07_141352.txt` - aktuální kontrolní audit po dorovnání projektové paměti: 0 kandidátů driftu, 21 aktivních připomenutí, záloha z 2026-08-02 stále vyžaduje nový běh a čistý lokální náskok je správně jen `GitHub batch pending`.
...
- `projects/camino.md`; `handoffs/workstreams/project-camino.md`; `tvbcp/workstreams/project-camino.md` — [PRIPOMENOUT] V0.5 + U15 jsou autoritativní. C03a čistý model, C03b lokální owner API v1 bez listeneru; C04a offline Trip/Day/Moment a C01c audio, C04b místní foto/video s explicitní vazbou komentáře a médií a obnovou rozpracovaného souboru. Swift 13/13, UI simulátoru a plná brána PASS. Nové Camino 0.1.0 (1) je podepsané, nainstalované a spuštěné na iPhonu 14 Plus; u T007 doložen úspěšný zápis a restart, celý T007 částečný; T008/T009/T011/T012/T013/T014 fyzicky PASS a bezpečná část T060 PASS. T009 potvrdil pravdivé odmítnutí kamery bez blokace Komentáře a deníku. T014 ověřil zámek, odchod z aplikace i přijatý hovor s přehratelnými pravdivě částečnými klipy, bez samovolného pokračování a bez hovoru v záznamu. Bezpečná T060 při běžném místě neměla falešné varování, video dokončila a zachovala starší položky. Skutečné nízké místo před/během audia, videa a textu zůstává NEOVĚŘENO. Read-only audit našel prahy jen u foto/videa, přímé čtení skutečné kapacity bez testovací injekce a žádné sledování tepelného stavu; hlavní iPhone se nemá uměle zaplňovat ani zahřívat. Synchronizace, Viewer a T096 neověřené; C04d/C05a navazují. C01a/C01b/C01c/C02a přijaty v prototypovém rozsahu. C02b T043/T047/T050 PASS synteticky, T049 v syntetické mobilní části; transfer build 0.4.0 (4) je nainstalovaný. Část A fyzicky PASS, část B po relaunchi s dostupnou sítí dokončila přenos, ale otevření bez sítě chybí. T048 má fyzický PASS tří dostupných syntetických scénářů na cizí Wi-Fi; captive portal zůstává NEOVĚŘENO. T048 i T049 jsou ukončené, Serve obnovený a Funnel vypnutý; plné T049 čeká na skutečné video. Viewer runtime/G8 a T061 v C05a zůstávají. Profil integrované aplikace do 27. 9. 2026, transfer harnessu do 24. 9. 2026, Camino Audio do 22. 9. 18:16 CEST.
...
- `handoffs/vocabulary_en_49_images_review_2026_09_06.md` - hotovo: 49 schválených ilustrací je začleněných a publikovaných na Pages z d22f25a4. Veřejný manifest 425 kartiček i SHA-256 všech 49 obrázků ověřen; důkaz v `reports/vocabulary_en_images_publish_2026_09_06.json`.
- `reports/vocabulary_en_mmtx_audit_2026_09_05.md` - rozšířený audit slovníčků i dialogů MMTX: 120 doplněných hesel s CZ, Sentence, SentenceT a Benji; celkem 425 karet, 239 Benji a 845 MP3. Doplnění včetně 49 nových ilustrací bylo veřejně ověřeno 2026-09-06.
...
- `handoffs/workstreams/project-vocabulary-fr.md`; `tvbcp/workstreams/project-vocabulary-fr.md` - FR aktualizace 13. 9.: Míla 234 slovíček (+7), Jana 413 kompletních párů vět (+7 doplnění). Společný audit tří slovníků a osm nových ilustrací; podrobnosti v `reports/vocabulary_fr_it_refresh_2026_09_13.md`.
- `handoffs/workstreams/project-vocabulary-it.md`; `tvbcp/workstreams/project-vocabulary-it.md` - IT aktualizace 13. 9.: 471 slovíček (+8) s příklady a obrázky. Společné mapování 1 082 vazeb, desktopové i Pythonista resolvery bez chybějících obrázků.
...
- `projects/mmtx_story_hotspot_app.md`; `handoffs/workstreams/project-mmtx.md`; `tvbcp/workstreams/project-mmtx.md` - MMTX 13. 9.: opravená navigace scén 1–5, dveře vpřed a šipky zpět; 63 testů, 18 browserových prokliků. Veřejně nasazeno: commit `0e1906b8`, Pages `34768695626`, shoda 10/10 souborů a 18 produkčních prokliků.
...
- `projects/to_be_to_have.md`; `handoffs/workstreams/project-to-be-to-have.md`; `tvbcp/workstreams/project-to-be-to-have.md` — ToBeToHave: schválený webový prototyp a publikační kopie `docs/to-be-to-have/`; přímý vstup Webové aplikace v horní liště Cockpitu. 14 JS + 22 Cockpit testů a 236 HTTP kontrol OK; finální publikaci a nasazení ověřovat živě.
...
- `projects/automated_recurring_tasks.md`; `handoffs/colors_numbers_owl_pages_artifact_checkpoint_2026_07_27.md` - automatické opakující se úkoly a ColorsAndNumbers soví TTS; od 21. 9. jeden řádek `default` zajišťuje denní MP3, pokud chybí přesné datum, které má vždy přednost. Produkční commit `b16cae26`, workflow `35567028291`; veřejný `app.js` a `owl_210926.mp3` HTTP 200 / `audio/mp3` / 78 624 B / 13,104 s. Pages artifact je živá publikační autorita a workflow nemění `main`.
...
- `projects/family_memory_films.md` - originály USA jsou od 2026-09-15 přesunuté na externí disk Falta (2 759 souborů, 33,10 GiB, SHA-256 ověřeno); před dalším výběrem upravit cestu k originálům a ověřit přehrávání. Historický stav: obecna platforma pro trideni rodinnych fotek/videi a pripravu vzpominkovych filmu; prvni dataset je USA 2019 na plose Macu, cisty seznam 15 dnu je odsouhlaseny, master prehled `Tomik 2` je ulozen mimo git, predstrihovy formular `03_overview/film_selection_form.html` ma autosave, CSV export, rating fotek/videi a prehravani videi a Adamuv prvni navrh ratingu je aplikovany.
...
- `tvbcp/architektura_komunikace_samantha.txt` - aktivni kanonicka
  ...
  tah bez vyvojoveho zameru nic nezapisuje. Implementace prosla plnou branou a
  rizene nasazeni `ef3526d` potvrdilo zivy zapisovaci status Samantha Agent/RAG
  i smoke 5/5.
...
- `handoffs/cockpit_email_archive_browser_2026_07_09.md` - Cockpit Archiv e-mailu: UX0+UX1, první ladění i UX2 pravdivé navigace jsou nasazené. E-maily mají vlastní rozcestník, ScanDocu je pod Dokumenty a pracovní schopnosti zmizely z katalogu Webových aplikací.
...
- `handoffs/knowledge_library_article_editing_2026_07_16.md` - Knihovna: fotografie na pozadí a správná pole kategorií; 2026-09-12 navazující rozpočet 305 KiB celkem (jeden hlavní JPEG + náhled). Nové ukládání nasazeno (`2f5cf1f9`, 1622 testů, smoke 5/5). Potvrzený převod 22 starších obrázků dokončen: 59,117 → 5,962 MiB. Jednorázová záloha 59,35 MiB je na výslovný pokyn smazaná, aktivní archiv zachovaný; důkazy v handoffu a `tvbcp/knihovna_cockpit.txt`.
