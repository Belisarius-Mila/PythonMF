...
- `projects/email_readonly_oauth.md` - e-mailová integrace Samanthy: read-only hledání, triage a archivace; UX0+UX1 i ladění řazení podle přijatého data, bezpečného textu z EML a přímého otevření celého PDF jsou nasazené. Brány a živý test prošly, zbývá iPhone vizuální retest.
...
- `handoffs/cockpit_email_archive_browser_2026_07_09.md` - Cockpit Archiv e-mailu: read-only prohlížeč EmailArchiveVault na `/email-archive/`; UX0+UX1 i následné ladění řazení podle přijatého data, úplnějšího bezpečného textu z EML a přímého otevření celého PDF na mobilu jsou nasazené a čekají na iPhone retest.
