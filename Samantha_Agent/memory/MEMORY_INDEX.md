...
- `projects/samantha_external_backup.md` - [PRIPOMENOUT] offline zálohování `PythonMF`/Samanthy na externí disk: recovery profil zálohuje celý `PythonMF` do šifrovaného kontejneru, stav hlídat přes `scripts/backup_status.py`; poslední záloha 2026-05-19 je po 3denním intervalu, disk je mimo dosah do pátku 2026-05-29.
...
- `handoffs/family_video_organizer_package_ready_2026_05_29.md` - [PRIPOMENOUT] FamilyVideoOrganizer: realny lehky ZIP pro dceru je pripraven mimo git, generator `tomik_family_video_package.py` vytvari `videos-data.js`/nahledy, UI ma Safari fallback, zelene tlacitko videi a zamykani radku; dalsi krok je poslat ZIP a pockat na export JSON.
...
- `handoffs/cockpit_dashboard_terminal_launch_checkpoint_2026_05_29.md` - [PRIPOMENOUT] Samantha Cockpit: dashboard Dnes/Stav/Akce, git status, tlacitka Samantha chat a Codex CLI jsou lokalne commitnute; branch je ahead 6, zustava necommitnuty `memory/contacts.md`, dalsi krok je potvrzeny push a samostatne rozhodnuti o kontaktech.
