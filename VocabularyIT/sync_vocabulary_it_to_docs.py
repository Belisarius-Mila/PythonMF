...
AMBIGUOUS_PRONOUNS = {
    "ja", "ty", "my", "vy", "io", "tu", "noi", "voi", "me",
}
CONJUNCTION_WORDS = {"a", "nebo", "ale", "e", "o", "ma", "pero"}
PREPOSITION_WORDS = {
    "na", "v", "ve", "do", "z", "u", "k", "od", "po", "pro", "s", "bez", "mezi",
    "su", "a", "da", "di", "in", "con", "per",
}
ADJ_ADV_WORDS = {"pridavnejmeno", "prislovce", "aggettivo", "avverbio"}
...
def is_probable_verb(word: str) -> bool:
    ...
    if not normalized or " " in normalized:
        return False
    return normalized.endswith(("are", "ere", "ire", "ar", "er", "ir", "at", "it", "et", "yt"))


def is_probable_adj_or_adv(it_word: str, cz_word: str) -> bool:
    cz = normalize_word(cz_word)
    it = normalize_word(it_word)
...
def pick_gender_for_ambiguous(it_word: str, cz_word: str) -> str:
    key = f"{normalize_word(it_word)}|{normalize_word(cz_word)}"
    if not key.strip("|"):
        return "man"
...
def repair_rows(rows: list[dict[str, str]]) -> list[dict[str, str]]:
    ...
    for row in rows:
        ...
        if not it and not cz and not sentence:
            continue
...
def mapping_file_candidates(base_dirs: list[Path], csv_path: Path) -> list[Path]:
    ...
    for base in base_dirs:
        ...
        candidates.append(base / "mapping_it.json")
    candidates.append(csv_path.parent.resolve() / "mapping.json")
    candidates.append(csv_path.parent.resolve() / "mapping_it.json")
    dedup: list[Path] = []
    seen: set[Path] = set()
...
def blocklist_file_candidates(base_dirs: list[Path], csv_path: Path) -> list[Path]:
    candidates: list[Path] = []
    for base in base_dirs:
        candidates.append(base / "mapping_it_blocklist.json")
    candidates.append(csv_path.parent.resolve() / "mapping_it_blocklist.json")
    dedup: list[Path] = []
    seen: set[Path] = set()
    for candidate in candidates:
        resolved = candidate.resolve()
...
def load_blocked_terms(base_dirs: list[Path], csv_path: Path) -> set[str]:
    blocked: set[str] = set()
    for path in blocklist_file_candidates(base_dirs, csv_path):
        if not path.exists():
            continue
        try:
            data = json.loads(path.read_text(encoding="utf-8"))
...
def choose_picture_stem(
    ...
    it = (row.get("IT") or "").strip()
    cz = (row.get("CZ") or "").strip()
    it_norm = normalize_word(it)
    cz_norm = normalize_word(cz)
    term_blocked = it_norm in blocked_terms
    tokens = tokenize_words(it) + tokenize_words(cz)
    generic_tokens = (
        FEMALE_PRONOUNS
        | MALE_PRONOUNS
        | AMBIGUOUS_PRONOUNS
    ...
    if token_set & AMBIGUOUS_PRONOUNS:
        return pick_gender_for_ambiguous(it, cz), "fallback"
    ...
    if is_probable_verb(it):
        return "verb", "fallback"
    if is_probable_adj_or_adv(it, cz):
        return "proverbs", "fallback"
...
def find_picture_path(stem: str, base_dirs: list[Path]) -> Path | None:
    ...
    for base_dir in base_dirs:
        ...
        for path in base_dir.iterdir():
            if path.suffix.lower() not in SUPPORTED_IMAGE_SUFFIXES:
                continue
            if normalize_word(path.stem) == normalized_stem:
                return path
    return None
...
def build_export_payload(
    ...
    for index, row in enumerate(rows, start=1):
        requested_stem, image_source = choose_picture_stem(row, picture_stems, synonym_image_map, blocked_terms)
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Synchronize VocabularyIT CSV and images into web-friendly docs assets."
    )
    parser.add_argument(
        "--csv-path",
        default=str(repo_root / "VocabularyIT" / CSV_FILENAME),
        help="Source CSV file.",
    )
    parser.add_argument(
        "--output-root",
        default=str(repo_root / "docs"),
        help="Primary web output root. The script writes data/vocabulary-it.json and assets/vocabulary-it/ here.",
    )
    parser.add_argument(
        "--json-name",
        default="vocabulary-it.json",
        help="Output JSON filename inside data/.",
    )
    parser.add_argument(
        "--asset-dir-name",
        default="vocabulary-it",
...
def main() -> int:
    ...
    blocked_terms = load_blocked_terms(base_dirs, csv_path)
