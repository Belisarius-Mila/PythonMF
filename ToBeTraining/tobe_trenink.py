...
COLUMN_QUESTION_AUX = "QuestionAux"
COLUMN_QUESTION_VERB = "QuestionVerb"
...
class ToBeTrainerApp:
    def __init__(self, master):
        self.master = master
        self.master.title("Trénink be / have / go – věty z CSV")
        ...
        lesson_frame = tk.Frame(self.main_frame)
        lesson_frame.place(relx=1.0, x=-20, y=20, anchor="ne")
        self.lesson_buttons = {}
        for col_idx, (button_text, lesson_key) in enumerate((
            ("to be", LESSON_TO_BE),
            ("to have", LESSON_TO_HAVE),
            ("to go", LESSON_TO_GO),
        )):
            button = tk.Button(
                lesson_frame,
                ...
                font=("Arial", 16, "bold"),
                width=10,
                command=lambda key=lesson_key: self.set_sentence_lesson(key),
            )
            button.grid(row=0, column=col_idx, padx=6, sticky="e")
            self.lesson_buttons[lesson_key] = button

        # Rámec pro text
        text_frame = tk.Frame(self.main_frame)
        text_frame.pack(pady=(90, 40), padx=40, fill="both", expand=True)
        ...
        self.set_sentence_lesson(self.current_lesson)
    ...
    def load_csv(self, filename):
        ...
        try:
            with open(filename, "r", encoding="utf-8-sig", newline="") as f:
                ...
                needed = {COLUMN_LESSON, COLUMN_QUESTION, COLUMN_POSITIVE, COLUMN_NEGATIVE}
                if not needed.issubset(set(reader.fieldnames or [])):
                    msg = (
                        "V CSV souboru chybí některý z požadovaných sloupců:\n"
                        f"{COLUMN_LESSON}, {COLUMN_QUESTION}, {COLUMN_POSITIVE}, {COLUMN_NEGATIVE}\n\n"
    ...
    def set_sentence_lesson(self, lesson_key: str):
        filtered = [row for row in self.all_sentences if row.get(COLUMN_LESSON) == lesson_key]
        if not filtered:
            messagebox.showerror("Chyba", f"Pro lekci '{lesson_key}' nejsou k dispozici žádné věty.")
            return
        self.current_lesson = lesson_key
        self.sentences = filtered
        self.indices = list(range(len(self.sentences)))
        random.shuffle(self.indices)
        self.current_pos = -1
        self._update_lesson_buttons()
        self.show_new_sentence()

    def _update_lesson_buttons(self):
        for lesson_key, button in self.lesson_buttons.items():
            if lesson_key == self.current_lesson:
                button.config(
                    relief="sunken",
                    bd=4,
                    state="disabled",
        ...
        try:
            with open(filename, "r", encoding="utf-8-sig", newline="") as f:
                ...
                needed = {
                    ...
                    COLUMN_ADVERBIAL,
                    COLUMN_QUESTION_AUX,
                    COLUMN_QUESTION_VERB,
                    COLUMN_TRANSLATION,
                }
                if not needed.issubset(set(reader.fieldnames or [])):
                    msg = (
                        "V CSV souboru chybí některý z požadovaných sloupců:\n"
                        f"{COLUMN_PRONOUN}, {COLUMN_VERB}, {COLUMN_ADVERBIAL}, "
                        f"{COLUMN_QUESTION_AUX}, {COLUMN_QUESTION_VERB}, {COLUMN_TRANSLATION}\n\n"
                        f"Nalezené sloupce: {reader.fieldnames}"
                    )
                    messagebox.showerror("Chyba struktury CSV", msg)
                    return []
                ...
                for row in reader:
                    ...
                    question_aux = (row.get(COLUMN_QUESTION_AUX) or "").strip()
                    question_verb = (row.get(COLUMN_QUESTION_VERB) or "").strip()
                    translation = (row.get(COLUMN_TRANSLATION) or "").strip()
                    if pronoun and verb and adverbial and question_verb and translation:
                        rows.append(
                            {
                                COLUMN_PRONOUN: pronoun,
                                COLUMN_VERB: verb,
                                COLUMN_ADVERBIAL: adverbial,
                                COLUMN_QUESTION_AUX: question_aux,
                                COLUMN_QUESTION_VERB: question_verb,
                                COLUMN_TRANSLATION: translation,
                            }
                        )
        except FileNotFoundError:
    ...
    def _verconjug_question_parts(self):
        if not self.verconjug_current:
            return "", "", "", ""
        return (
            self.verconjug_current[COLUMN_QUESTION_AUX],
            self.verconjug_current[COLUMN_PRONOUN],
            self.verconjug_current[COLUMN_QUESTION_VERB],
            self.verconjug_current[COLUMN_ADVERBIAL],
        )

    def _build_verconjug_question_sentence(self):
        aux, pronoun, question_verb, adverbial = self._verconjug_question_parts()
        if aux:
            parts = [part for part in (aux, pronoun, question_verb, adverbial) if part]
        else:
            parts = [part for part in (question_verb, pronoun, adverbial) if part]
        return " ".join(parts) + "?"
    ...
    def _verconjug_run_question_mode(self):
        ...
        question_aux = self.verconjug_current[COLUMN_QUESTION_AUX]
        question_verb = self.verconjug_current[COLUMN_QUESTION_VERB]
        pronoun_bounds = self._verconjug_box_bounds(self.verconjug_pronoun_label)

        self.verconjug_translation.config(text="")
        self.verconjug_adverbial_label.config(text=adverbial_text)

        is_to_be_question = not question_aux

        if is_to_be_question:
            self.verconjug_pronoun_label.config(text="")
            self.verconjug_verb_label.config(text="")
            ...
            px, py, pw, ph = pronoun_bounds
            self.verconjug_verb_label.config(text=verb_text)
            pronoun_box = None
            pronoun_start = None
            pronoun_target = None
            verb_box = None
            verb_start = None
            verb_target = None
            aux_box, aux_target = self._create_floating_box(self.verconjug_pronoun_label, question_aux)
            gap = 20
            start_x = max(0, px - pw - 260)
            aux_start = (start_x, py, pw, ph)
            aux_target = (max(0, px - pw - gap), py, pw, ph)
            aux_box.place(x=start_x, y=py, width=pw, height=ph)
        ...
        def animate(step=0):
            ...
            if pronoun_box is not None and pronoun_start is not None and pronoun_target is not None:
                move_widget(pronoun_box, pronoun_start, pronoun_target, progress)
            if verb_box is not None and verb_start is not None and verb_target is not None:
                move_widget(verb_box, verb_start, verb_target, progress)
            if aux_box is not None and aux_start is not None and aux_target is not None:
                move_widget(aux_box, aux_start, aux_target, progress)
            if progress >= 1.0:
                if is_to_be_question:
                    self._clear_verconjug_animation()
                    ...
                    self.verconjug_pronoun_label.config(text=pronoun_text)
                    self.verconjug_verb_label.config(text=question_verb)
                self.verconjug_adverbial_label.config(text=adverbial_text)
    ...
    def _verconjug_speak_question_sentence(self):
        if not self.verconjug_running or not self.verconjug_current:
            return
        sentence = self._build_verconjug_question_sentence()
        speak_english(sentence)
