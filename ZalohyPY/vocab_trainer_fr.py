    ...
    if getattr(sys, "frozen", False) or _is_macos_app_runtime():
        ...
        with open(target_csv, "w", encoding="utf-8", newline="") as f:
            writer = csv.DictWriter(
                f, fieldnames=["FR", "CZ", "Order", "Sentence", "SentenceT", "L", "HT"]
            )
            writer.writeheader()
        return target_csv
...
class VocabularyTrainerApp:
    ...
    def _build_ui(self):
        ...
        tk.Button(
            controls, text="Image", command=self.export_actual_words_image, width=10
    ...
    def _write_rows(self, rows):
        fieldnames = ["FR", "CZ", "Order", "Sentence", "SentenceT", "L", "HT"]
    ...
    def export_actual_words_image(self):
        ...
        for i in indices:
            ...
            fr = (row.get("FR") or "").strip()
            cz = (row.get("CZ") or "").strip()
            if not fr and not cz:
                continue
            data_rows.append((fr, cz))
        ...
        prepared = []
        content_h = 0
        for fr, cz in data_rows:
            fr_lines = self._wrap_text_for_image(probe_draw, fr, body_font, col_w)
            cz_lines = self._wrap_text_for_image(probe_draw, cz, body_font, col_w)
            row_lines = max(len(fr_lines), len(cz_lines))
            row_h = row_lines * line_h + row_pad_y * 2
            prepared.append((fr_lines, cz_lines, row_h))
            content_h += row_h
        ...
        draw.text((x_fr, y), "FR", font=title_font, fill="black")
        draw.text((x_cz, y), "CZ", font=title_font, fill="black")
        y += header_h

        for fr_lines, cz_lines, row_h in prepared:
            for li, line in enumerate(fr_lines):
                draw.text((x_fr, y + row_pad_y + li * line_h), line, font=body_font, fill="black")
    ...
    def open_input_window(self):
        ...
        columns = ("Order", "FR", "CZ", "Sentence", "SentenceT", "L", "HT")
        tree = ttk.Treeview(
            list_frame,
            columns=columns,
            show="headings",
