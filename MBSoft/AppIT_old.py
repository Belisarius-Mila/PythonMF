...
class VocabTrainer(ui.View):
    ...
    def refresh_data(self, sender):
        ...
        if result.get("unchanged"):
            lines.append('Už aktuální: ' + ', '.join(result["unchanged"]))
        if result["missing"]:
    ...
    def will_close(self):
        self.stop_auto(None)
        # Return user to launcher after closing app view.
        ui.delay(self._open_launcher_editor, 0.15)
